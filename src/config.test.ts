...
describe("loadConfig", () => {
	...
	beforeEach(async () => {
		tempDir = await mkdtemp(join(tmpdir(), "codexstory-test-"));
	});
...
`);
		await Bun.write(
			join(tempDir, ".codexstory", "config.local.yaml"),
			`agents:\n  maxConcurrent: 4\n`,
		);
...
describe("validateConfig", () => {
	...
	async function writeConfig(yaml: string): Promise<void> {
		await Bun.write(join(tempDir, ".codexstory", "config.yaml"), yaml);
	}
...
describe("resolveProjectRoot", () => {
	...
	test("returns startDir when .codexstory/config.yaml exists there", async () => {
		repoDir = await createTempGitRepo();
		await mkdir(join(repoDir, ".codexstory"), { recursive: true });
		await Bun.write(
			join(repoDir, ".codexstory", "config.yaml"),
			"project:\n  canonicalBranch: main\n",
		);
	...
	test("resolves worktree to main project root", async () => {
		...
		// Create a worktree like codexstory sling does
		const worktreeDir = join(repoDir, ".codexstory", "worktrees", "test-agent");
		await mkdir(join(repoDir, ".codexstory", "worktrees"), { recursive: true });
	...
	test("resolves worktree to main root even when config.yaml is committed (regression)", async () => {
		...
		// Commit .codexstory/config.yaml so the worktree gets a copy via git
		// (this is what codexstory init does — the file is tracked)
		await mkdir(join(repoDir, ".codexstory"), { recursive: true });
		await Bun.write(
			join(repoDir, ".codexstory", "config.yaml"),
			"project:\n  canonicalBranch: main\n",
		);
		await runGitInDir(repoDir, ["add", ".codexstory/config.yaml"]);
		await runGitInDir(repoDir, ["commit", "-m", "add codexstory config"]);

		// Create a worktree — it will now have .codexstory/config.yaml from git
		const worktreeDir = join(repoDir, ".codexstory", "worktrees", "mail-scout");
		await mkdir(join(repoDir, ".codexstory", "worktrees"), { recursive: true });
		await runGitInDir(repoDir, [
			"worktree",
			"add",
			"-b",
			"codexstory/mail-scout/task-1",
			worktreeDir,
		]);

		// Must resolve to main repo root, NOT the worktree
		// (even though worktree has its own .codexstory/config.yaml)
		const result = await resolveProjectRoot(worktreeDir);
		expect(result).toBe(repoDir);
	});
	...
	test("loadConfig resolves correct root from worktree", async () => {
		...
		const worktreeDir = join(repoDir, ".codexstory", "worktrees", "agent-2");
		await mkdir(join(repoDir, ".codexstory", "worktrees"), { recursive: true });
		await runGitInDir(repoDir, ["worktree", "add", "-b", "codexstory/agent-2/task-2", worktreeDir]);
