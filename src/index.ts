...
/**
 * Codexstory CLI — main entry point and command router.
 *
 * Routes subcommands to their respective handlers in src/commands/.
 * Usage: codexstory <command> [args...]
 */
...
const HELP = `codexstory v${VERSION} — Multi-agent orchestration for Codex CLI

Usage: codexstory <command> [args...]

Commands:
  agents <sub>            Discover and query agents (discover)
  init                    Initialize .codexstory/ in current project
...
Run 'codexstory <command> --help' for command-specific help.`;
...
async function main(): Promise<void> {
	...
	if (command === "--version" || command === "-v") {
		process.stdout.write(`codexstory v${VERSION}\n`);
		return;
	}
	...
	switch (command) {
		...
		default: {
			...
			process.stderr.write(`Run 'codexstory --help' for usage.\n`);
			process.exit(1);
		}
	}
}
...
main().catch((err: unknown) => {
	...
	if (err instanceof WorktreeError && err.message.includes("not a git repository")) {
		process.stderr.write("Not in an codexstory project. Run 'codexstory init' first.\n");
		process.exit(1);
	}
