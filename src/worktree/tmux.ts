/**
 * Tmux session management for codexstory agent workers.
 *
 * All operations use Bun.spawn to call the tmux CLI directly.
 * Session naming convention: `codexstory-{projectName}-{agentName}`.
 * The project name prefix prevents cross-project tmux session collisions
 * and enables project-scoped cleanup (codexstory-pcef).
 */
...
/**
 * Detect the directory containing the codexstory binary.
 *
 ...
 * to `which codexstory` to find it on the current PATH.
 *
 * Returns null if detection fails.
 */
async function detectOverstoryBinDir(): Promise<string | null> {
	// process.argv[1] is the script entry point (e.g., /path/to/codexstory/src/index.ts)
	// The codexstory binary (bun link) resolves to a bin dir
	// Try `which codexstory` for the most reliable result
	try {
		const proc = Bun.spawn(["which", "codexstory"], {
			stdout: "pipe",
			stderr: "pipe",
		});
		...
		// which not available or codexstory not on PATH
	}

	// Fallback: if process.argv[1] points to codexstory's own entry point (src/index.ts),
	// derive the bin dir from the bun binary that's running it
	const scriptPath = process.argv[1];
	if (scriptPath?.includes("codexstory")) {
 ...
 *
 * @param name - Session name (e.g., "codexstory-myproject-auth-login")
...
export async function createSession(
	...
	// Ensure PATH includes the codexstory binary directory
	// so that hooks calling `codexstory` inside the session can find it
 ...
 * Used by `codexstory prime` to register the orchestrator's tmux session
