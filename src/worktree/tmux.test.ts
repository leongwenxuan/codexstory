...
describe("createSession", () => {
	...
	test("creates session and returns pane PID", async () => {
		...
		spawnSpy.mockImplementation(() => {
			callCount++;
			if (callCount === 1) {
				// which codexstory — return a bin path
				return mockSpawnResult("/usr/local/bin/codexstory\n", "", 0);
			}
			if (callCount === 2) {
				// tmux new-session
				return mockSpawnResult("", "", 0);
			}
			// tmux list-panes -t codexstory-auth -F '#{pane_pid}'
	...
	test("passes correct args to tmux new-session with PATH wrapping", async () => {
		...
		// Call 0 is 'which codexstory', call 1 is 'tmux new-session'
	...
	test("calls list-panes after creating to get pane PID", async () => {
		...
		// 3 calls: which codexstory, tmux new-session, tmux list-panes
	...
	test("still creates session when which codexstory fails (uses fallback)", async () => {
		let callCount = 0;
		spawnSpy.mockImplementation(() => {
			callCount++;
			if (callCount === 1) {
				// which codexstory fails
				return mockSpawnResult("", "codexstory not found", 1);
			}
			if (callCount === 2) {
				// tmux new-session
				return mockSpawnResult("", "", 0);
...
describe("listSessions", () => {
	...
	test("parses session list output", async () => {
		spawnSpy.mockImplementation(() =>
			mockSpawnResult("codexstory-auth:42\noverstory-data:99\n", "", 0),
		);

		const sessions = await listSessions();

		expect(sessions).toHaveLength(2);
		expect(sessions[0]?.name).toBe("codexstory-auth");
		expect(sessions[0]?.pid).toBe(42);
		expect(sessions[1]?.name).toBe("codexstory-data");
...
describe("getPanePid", () => {
	...
	test("returns PID from tmux display-message", async () => {
		...
		const pid = await getPanePid("codexstory-auth");

		expect(pid).toBe(42);
		const callArgs = spawnSpy.mock.calls[0] as unknown[];
		const cmd = callArgs[0] as string[];
		expect(cmd).toEqual(["tmux", "display-message", "-p", "-t", "codexstory-auth", "#{pane_pid}"]);
	});
...
describe("killSession", () => {
	...
	test("gets pane PID, kills process tree, then kills tmux session", async () => {
		...
		await killSession("codexstory-auth");
		...
		expect(lastCmd).toEqual(["tmux", "kill-session", "-t", "codexstory-auth"]);
	...
	test("succeeds silently when session is already gone after process cleanup", async () => {
		spawnSpy.mockImplementation((...args: unknown[]) => {
			...
			if (cmd[0] === "tmux" && cmd[1] === "kill-session") {
				...
				return mockSpawnResult("", "can't find session: codexstory-auth", 1);
			}
			return mockSpawnResult("", "", 0);
		});
...
describe("isSessionAlive", () => {
	...
	test("returns true when session exists (exit 0)", async () => {
		...
		const alive = await isSessionAlive("codexstory-auth");
...
describe("sendKeys", () => {
	...
	test("passes correct args to tmux send-keys", async () => {
		...
		await sendKeys("codexstory-auth", "echo hello world");

		expect(spawnSpy).toHaveBeenCalledTimes(1);
		const callArgs = spawnSpy.mock.calls[0] as unknown[];
		const cmd = callArgs[0] as string[];
		expect(cmd).toEqual(["tmux", "send-keys", "-t", "codexstory-auth", "echo hello world", "Enter"]);
	});

	test("flattens newlines in keys to spaces", async () => {
		spawnSpy.mockImplementation(() => mockSpawnResult("", "", 0));

		await sendKeys("codexstory-agent", "line1\nline2\nline3");
	...
	test("sends Enter with empty string (follow-up submission)", async () => {
		...
		await sendKeys("codexstory-agent", "");

		expect(spawnSpy).toHaveBeenCalledTimes(1);
		const callArgs = spawnSpy.mock.calls[0] as unknown[];
		const cmd = callArgs[0] as string[];
		expect(cmd).toEqual(["tmux", "send-keys", "-t", "codexstory-agent", "", "Enter"]);
	});
	...
	test("throws descriptive error when tmux server is not running", async () => {
		...
		await expect(sendKeys("codexstory-agent-fake", "hello")).rejects.toThrow(
	...
	test("throws descriptive error when session not found", async () => {
		spawnSpy.mockImplementation(() =>
			mockSpawnResult("", "cant find session: codexstory-agent-fake\n", 1),
		);
		await expect(sendKeys("codexstory-agent-fake", "hello")).rejects.toThrow(/does not exist/);
	});

	test("throws generic error for other failures", async () => {
		spawnSpy.mockImplementation(() => mockSpawnResult("", "some other error\n", 1));
		await expect(sendKeys("codexstory-agent-fake", "hello")).rejects.toThrow(/Failed to send keys/);
	});
});
...
describe("waitForTuiReady", () => {
	...
	test("returns false when timeout expires without content", async () => {
		...
		const ready = await waitForTuiReady("codexstory-agent", 2_000, 500);
	...
	test("uses default timeout and poll interval", async () => {
		...
		const ready = await waitForTuiReady("codexstory-agent");
