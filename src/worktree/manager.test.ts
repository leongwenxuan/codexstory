...
describe("createWorktree", () => {
	...
	beforeEach(async () => {
		...
		worktreesDir = join(repoDir, ".codexstory", "worktrees");
	...
	test("returns correct path and branch name", async () => {
		...
		expect(result.branch).toBe("codexstory/auth-login/bead-abc123");
	});
	...
	test("creates the branch in the repo", async () => {
		...
		expect(branchList).toContain("codexstory/auth-login/bead-abc123");
	});
	...
	test("WorktreeError includes worktree path and branch name", async () => {
		...
		try {
			...
			expect(wtErr.branchName).toBe("codexstory/auth-login/bead-abc123");
		}
	});
});
...
describe("listWorktrees", () => {
	...
	test("lists multiple worktrees after creation", async () => {
		...
		expect(branches).toContain("codexstory/auth-login/bead-abc");
		expect(branches).toContain("codexstory/data-sync/bead-xyz");
	});
	...
	test("strips refs/heads/ prefix from branch names", async () => {
		...
		expect(worktreeEntry?.branch).toBe("codexstory/feature-worker/bead-123");
	...
	test("throws WorktreeError for non-git directory", async () => {
		...
		const tmpDir = realpathSync(await mkdtemp(join(tmpdir(), "codexstory-notgit-")));
		try {
...
describe("removeWorktree", () => {
	...
	test("deletes the associated branch after removal", async () => {
		...
		const branchList = await git(repoDir, ["branch", "--list"]);
		expect(branchList).not.toContain("codexstory/auth-login/bead-abc");
	});
	...
	test("without forceBranch, unmerged branch deletion is silently ignored", async () => {
		...
		const branchList = await git(repoDir, ["branch", "--list"]);
		expect(branchList).toContain("codexstory/auth-login/bead-abc");
	});
});
