...
// Helper to create a temp codexstory dir with config.yaml
function createTempOverstoryDir(configYaml: string): string {
	const tempDir = mkdtempSync(join(tmpdir(), "codexstory-test-"));
...
const mockConfig: OverstoryConfig = {
	...
	agents: {
		manifestPath: `${tmpdir()}/.codexstory/agent-manifest.json`,
		baseDir: `${tmpdir()}/.codexstory/agents`,
		maxConcurrent: 5,
		staggerDelayMs: 1000,
		maxDepth: 2,
		maxSessionsPerRun: 0,
	...
	worktrees: {
		baseDir: `${tmpdir()}/.codexstory/worktrees`,
	},
	taskTracker: {
		backend: "auto",
		enabled: false,
