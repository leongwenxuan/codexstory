...
describe("checkStructure", () => {
	...
	test("fails when .codexstory/ directory does not exist", async () => {
		const checks = await checkStructure(mockConfig, overstoryDir);

		expect(checks.length).toBeGreaterThan(0);
		const dirCheck = checks.find((c) => c.name === ".codexstory/ directory");
	...
	test("passes when all required files and directories exist", async () => {
		// Create .codexstory/ and all required structure
