...
function createTempGitRepo(): string {
	const dir = mkdtempSync(join(tmpdir(), "codexstory-test-"));
...
describe("checkConsistency", () => {
	...
	beforeEach(() => {
		...
		overstoryDir = join(repoRoot, ".codexstory");
	...
	test("detects orphaned worktrees", async () => {
		...
		createWorktree(repoRoot, worktreePath, "codexstory/orphan-agent/test-123");
	...
	test("detects orphaned tmux sessions", async () => {
		...
		mockListSessions.mockResolvedValue([{ name: "codexstory-testproject-orphan", pid: 9999 }]);
	...
	test("ignores tmux sessions from other projects", async () => {
		...
		mockListSessions.mockResolvedValue([
			{ name: "codexstory-otherproject-agent1", pid: 9999 },
	...
	test("detects dead PIDs in SessionStore", async () => {
		...
		store.upsert({
			...
			branchName: "codexstory/dead-agent/test-123",
			beadId: "test-123",
			tmuxSession: "codexstory-testproject-dead-agent",
			state: "working",
	...
	test("passes when all PIDs are alive", async () => {
		...
		store.upsert({
			...
			branchName: "codexstory/live-agent/test-123",
			beadId: "test-123",
			tmuxSession: "codexstory-testproject-live-agent",
			state: "working",
			pid: 12345,
			parentAgent: null,
			depth: 0,
	...
	test("detects missing worktrees for SessionStore entries", async () => {
		...
		store.upsert({
			...
			branchName: "codexstory/missing-agent/test-123",
			beadId: "test-123",
			tmuxSession: "codexstory-testproject-missing-agent",
			state: "working",
			pid: null,
			parentAgent: null,
			depth: 0,
	...
	test("detects missing tmux sessions for SessionStore entries", async () => {
		...
		createWorktree(repoRoot, worktreePath, "codexstory/agent-without-tmux/test-123");
		...
		store.upsert({
			...
			capability: "builder",
			worktreePath,
			branchName: "codexstory/agent-without-tmux/test-123",
			beadId: "test-123",
			tmuxSession: "codexstory-testproject-agent-without-tmux",
			state: "working",
			pid: null,
			parentAgent: null,
			depth: 0,
	...
	test("passes when everything is consistent", async () => {
		...
		createWorktree(repoRoot, worktreePath, "codexstory/consistent-agent/test-123");
		...
		store.upsert({
			...
			capability: "builder",
			worktreePath,
			branchName: "codexstory/consistent-agent/test-123",
			beadId: "test-123",
			tmuxSession: "codexstory-testproject-consistent-agent",
			state: "working",
			pid: 12345,
			parentAgent: null,
			depth: 0,
		...
		mockListSessions.mockResolvedValue([
			{ name: "codexstory-testproject-consistent-agent", pid: 12345 },
		]);
	...
	test("fails early if SessionStore cannot be opened", async () => {
		// Use a bad codexstory directory path
		const badOverstoryDir = "/nonexistent/.codexstory";
	...
	test("reviewer-coverage: leads without reviewers emits warn", async () => {
		...
		store.upsert({
			...
			branchName: "codexstory/builder-1/test-123",
			beadId: "test-123",
			tmuxSession: "codexstory-testproject-builder-1",
			state: "working",
			pid: null,
			parentAgent: "lead-1",
			depth: 1,
		...
		store.upsert({
			...
			branchName: "codexstory/builder-2/test-456",
			beadId: "test-456",
			tmuxSession: "codexstory-testproject-builder-2",
			state: "working",
			pid: null,
			parentAgent: "lead-1",
			depth: 1,
	...
	test("reviewer-coverage: partial reviewer coverage emits warn", async () => {
		...
		for (let i = 1; i <= 3; i++) {
			store.upsert({
				...
				branchName: `codexstory/builder-${i}/test-${i}`,
				beadId: `test-${i}`,
				tmuxSession: `codexstory-testproject-builder-${i}`,
				state: "working",
				pid: null,
				parentAgent: "lead-1",
				depth: 1,
		...
		store.upsert({
			...
			branchName: "codexstory/reviewer-1/test-r1",
			beadId: "test-r1",
			tmuxSession: "codexstory-testproject-reviewer-1",
			state: "working",
			pid: null,
			parentAgent: "lead-1",
			depth: 1,
	...
	test("reviewer-coverage: full reviewer coverage emits pass", async () => {
		...
		for (let i = 1; i <= 2; i++) {
			...
			store.upsert({
				...
				branchName: `codexstory/reviewer-${i}/test-r${i}`,
				beadId: `test-r${i}`,
				tmuxSession: `codexstory-testproject-reviewer-${i}`,
				state: "working",
				pid: null,
				parentAgent: "lead-1",
				depth: 1,
	...
	test("reviewer-coverage: multiple leads mixed coverage", async () => {
		...
		store.upsert({
			...
			capability: "builder",
			worktreePath: join(overstoryDir, "worktrees", "builder-1"),
			branchName: "codexstory/builder-1/test-1",
			beadId: "test-1",
			tmuxSession: "codexstory-testproject-builder-1",
			state: "working",
			pid: null,
		...
		store.upsert({
			...
			capability: "builder",
			worktreePath: join(overstoryDir, "worktrees", "builder-2"),
			branchName: "codexstory/builder-2/test-2",
			beadId: "test-2",
			tmuxSession: "codexstory-testproject-builder-2",
			state: "working",
			pid: null,
