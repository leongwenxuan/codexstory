 ...
 * Validates codexstory CLI version, config schema version, database schema versions.
 */
export const checkVersion: DoctorCheckFn = async (
	...
	// Determine codexstory tool root (not the target project)
...
async function checkCurrentVersion(toolRoot: string): Promise<DoctorCheck> {
	try {
		...
		return {
			...
			category: "version",
			status: "pass",
			message: `codexstory v${packageJson.version}`,
		};
	} catch (error) {
		return {
			name: "version-current",
