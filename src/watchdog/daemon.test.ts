...
/** Create a temp directory with .codexstory/ subdirectory, ready for sessions.db. */
async function createTempRoot(): Promise<string> {
	const dir = await mkdtemp(join(tmpdir(), "codexstory-daemon-test-"));
	await mkdir(join(dir, ".codexstory"), { recursive: true });
	return dir;
}
...
function writeSessionsToStore(root: string, sessions: AgentSession[]): void {
	const dbPath = join(root, ".codexstory", "sessions.db");
...
function makeSession(overrides: Partial<AgentSession> = {}): AgentSession {
	return {
		...
		branchName: "codexstory/test-agent/test-task",
		beadId: "test-task",
		tmuxSession: "codexstory-test-agent",
		state: "working",
...
describe("daemon tick", () => {
	...
	test("tick with dead tmux transitions session to zombie and fires terminate", async () => {
		const session = makeSession({
			...
			tmuxSession: "codexstory-dead-agent",
			state: "working",
			lastActivity: new Date().toISOString(),
		});
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-dead-agent": false });
		const checks: HealthCheck[] = [];
	...
	test("tick with alive tmux but zombie-old activity calls killSession", async () => {
		...
		const session = makeSession({
			...
			tmuxSession: "codexstory-zombie-agent",
			state: "working",
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-zombie-agent": true });
		const checks: HealthCheck[] = [];
		...
		expect(tmuxMock.killed).toContain("codexstory-zombie-agent");
	...
	test("first tick with stalled agent sets stalledSince and stays at level 0 (warn)", async () => {
		...
		const session = makeSession({
			...
			tmuxSession: "codexstory-stalled-agent",
			state: "working",
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-stalled-agent": true });
		const checks: HealthCheck[] = [];
	...
	test("stalled agent at level 2 calls triage when tier1Enabled", async () => {
		...
		expect(tmuxMock.killed).toContain("codexstory-stalled-agent");
		const reloaded = readSessionsFromStore(tempRoot);
		expect(reloaded[0]?.state).toBe("zombie");
	});
	...
	test("stalled agent at level 3 is terminated", async () => {
		...
		const session = makeSession({
			...
			tmuxSession: "codexstory-doomed-agent",
			state: "stalled",
			lastActivity: staleActivity,
			escalationLevel: 2,
			stalledSince,
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-doomed-agent": true });
		...
		// Level 3 = terminate
		expect(tmuxMock.killed).toContain("codexstory-doomed-agent");
	...
	test("triage retry sends nudge with recovery message", async () => {
		...
		const session = makeSession({
			...
			tmuxSession: "codexstory-retry-agent",
			state: "stalled",
			lastActivity: staleActivity,
			escalationLevel: 1,
			stalledSince,
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-retry-agent": true });
		const nudgeMock = nudgeTracker();
	...
	test("agent recovery resets escalation tracking", async () => {
		...
		const session = makeSession({
			...
			tmuxSession: "codexstory-recovered-agent",
			state: "working",
	...
	test("session persistence round-trip: load, modify, save, reload", async () => {
		const sessions: AgentSession[] = [
			makeSession({
				...
				tmuxSession: "codexstory-agent-alpha",
				state: "working",
				lastActivity: new Date().toISOString(),
			}),
			makeSession({
				...
				tmuxSession: "codexstory-agent-beta",
				state: "working",
			...
			makeSession({
				...
				tmuxSession: "codexstory-agent-gamma",
				state: "completed",
				lastActivity: new Date().toISOString(),
			}),
		];
		...
		const tmuxMock = tmuxWithLiveness({
			"codexstory-agent-alpha": true,
			"codexstory-agent-beta": false, // Dead — should become zombie
			"codexstory-agent-gamma": true, // Doesn't matter — completed is skipped
		});
	...
	test("multiple sessions with mixed states are all processed", async () => {
		...
		const sessions: AgentSession[] = [
			makeSession({
				id: "s1",
				agentName: "healthy",
				tmuxSession: "codexstory-healthy",
				state: "working",
		...
		const tmuxMock = tmuxWithLiveness({
			"codexstory-healthy": true,
			"codexstory-dying": false,
			"codexstory-stale": true,
			"codexstory-done": false,
		});
	...
	test("sessions with default escalation fields are processed correctly", async () => {
		...
		const session = makeSession({
			...
			agentName: "old-agent",
			worktreePath: "/tmp/test",
			branchName: "codexstory/old-agent/task",
			beadId: "task",
			tmuxSession: "codexstory-old-agent",
			state: "working",
			pid: process.pid,
			escalationLevel: 0,
			stalledSince: null,
...
describe("daemon event recording", () => {
	...
	function readEvents(root: string): StoredEvent[] {
		const dbPath = join(root, ".codexstory", "events.db");
	...
	test("escalation level 0 (warn) records event with type=escalation", async () => {
		...
		const eventsDbPath = join(tempRoot, ".codexstory", "events.db");
		...
		try {
			await runDaemonTick({
				...
				...THRESHOLDS,
				nudgeIntervalMs: 60_000,
				_tmux: tmuxWithLiveness({ "codexstory-stalled-agent": true }),
				_triage: triageAlways("extend"),
				_nudge: nudgeTracker().nudge,
	...
	test("escalation level 3 (terminate) records event with level=error", async () => {
		...
		try {
			await runDaemonTick({
				...
				...THRESHOLDS,
				nudgeIntervalMs: 60_000,
				_tmux: tmuxWithLiveness({ "codexstory-doomed-agent": true }),
				_triage: triageAlways("extend"),
				_nudge: nudgeTracker().nudge,
				_eventStore: eventStore,
			});
	...
	test("run_id is included in events when current-run.txt exists", async () => {
		...
		await Bun.write(join(tempRoot, ".codexstory", "current-run.txt"), runId);
...
describe("daemon mulch failure recording", () => {
	...
	test("Tier 0: recordFailure called when action=terminate (process death)", async () => {
		const session = makeSession({
			...
			capability: "builder",
			beadId: "task-123",
			tmuxSession: "codexstory-dying-agent",
			state: "working",
			lastActivity: new Date().toISOString(),
		});
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-dying-agent": false });
	...
	test("Tier 1: recordFailure called when triage returns terminate", async () => {
		...
		const session = makeSession({
			...
			capability: "scout",
			beadId: "task-456",
			tmuxSession: "codexstory-triaged-agent",
			state: "stalled",
			lastActivity: staleActivity,
			escalationLevel: 1,
			stalledSince,
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-triaged-agent": true });
		const failureMock = failureTracker();
	...
	test("recordFailure not called when triage returns extend", async () => {
		...
		const session = makeSession({
			...
			tmuxSession: "codexstory-extend-agent",
			state: "stalled",
			lastActivity: staleActivity,
			escalationLevel: 1,
			stalledSince,
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-extend-agent": true });
		const failureMock = failureTracker();
	...
	test("recordFailure includes evidenceBead when beadId is present", async () => {
		const session = makeSession({
			...
			capability: "builder",
			beadId: "task-789",
			tmuxSession: "codexstory-beaded-agent",
			state: "working",
			lastActivity: new Date().toISOString(),
		});
		...
		const tmuxMock = tmuxWithLiveness({ "codexstory-beaded-agent": false });
		const failureMock = failureTracker();
...
describe("run completion detection", () => {
	...
	test("nudges coordinator when all workers completed", async () => {
		const sessions = [
			makeSession({
				...
				capability: "builder",
				tmuxSession: "codexstory-agent-fake-builder-one",
				state: "completed",
				runId,
				lastActivity: new Date().toISOString(),
			}),
			makeSession({
				...
				capability: "builder",
				tmuxSession: "codexstory-agent-fake-builder-two",
				state: "completed",
				runId,
				lastActivity: new Date().toISOString(),
			}),
			makeSession({
				...
				tmuxSession: "codexstory-agent-fake-coordinator",
				state: "working",
				runId,
				lastActivity: new Date().toISOString(),
			}),
	...
	test("does not nudge when already notified (dedup marker)", async () => {
		...
		await Bun.write(join(tempRoot, ".codexstory", "run-complete-notified.txt"), runId);
	...
	test("ignores coordinator and monitor sessions for completion check", async () => {
		const sessions = [
			...
			makeSession({
				...
				agentName: "monitor",
				capability: "monitor",
				tmuxSession: "codexstory-agent-fake-monitor",
				state: "working",
				runId,
				lastActivity: new Date().toISOString(),
			}),
	...
	test("writes dedup marker after nudging", async () => {
		...
		const markerFile = Bun.file(join(tempRoot, ".codexstory", "run-complete-notified.txt"));
	...
	test("scout-only completion sends phase-appropriate message", async () => {
		const sessions = [
			makeSession({
				...
				agentName: "scout-one",
				capability: "scout",
				tmuxSession: "codexstory-agent-fake-scout-one",
				state: "completed",
				runId,
				lastActivity: new Date().toISOString(),
			}),
			makeSession({
				...
				agentName: "scout-two",
				capability: "scout",
				tmuxSession: "codexstory-agent-fake-scout-two",
				state: "completed",
				runId,
				lastActivity: new Date().toISOString(),
			}),
	...
	test("reviewer-only completion sends review-specific message", async () => {
		const sessions = [
			makeSession({
				...
				capability: "reviewer",
				tmuxSession: "codexstory-agent-fake-reviewer-one",
				state: "completed",
				runId,
				lastActivity: new Date().toISOString(),
			}),
