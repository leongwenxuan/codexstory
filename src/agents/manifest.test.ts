...
describe("createManifestLoader", () => {
	...
	beforeEach(async () => {
		tempDir = await mkdtemp(join(tmpdir(), "codexstory-manifest-test-"));
...
describe("resolveModel", () => {
	...
	function makeConfig(
		...
		return {
			...
			worktrees: { baseDir: ".codexstory/worktrees" },
...
describe("manifest validation accepts arbitrary model strings", () => {
	...
	beforeEach(async () => {
		tempDir = await mkdtemp(join(tmpdir(), "codexstory-model-test-"));
		manifestPath = join(tempDir, "agent-manifest.json");
		agentBaseDir = join(tempDir, "agents");
		await mkdir(agentBaseDir, { recursive: true });
	});
