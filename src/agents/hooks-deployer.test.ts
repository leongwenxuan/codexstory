...
describe("deployHooks", () => {
	...
	beforeEach(async () => {
		tempDir = await mkdtemp(join(tmpdir(), "codexstory-hooks-test-"));
	});
	...
	test("creates .codex/settings.local.json in worktree directory", async () => {
		const worktreePath = join(tempDir, "worktree");

		await deployHooks(worktreePath, "test-agent");

		const outputPath = join(worktreePath, ".codex", "settings.local.json");
	...
	test("PostToolUse hook includes debounced mail check entry", async () => {
		...
		expect(postToolUse[0].hooks[0].command).toContain("codexstory log tool-end");
		// Second entry is the debounced mail check
		expect(postToolUse[1].hooks[0].command).toContain("codexstory mail check --inject");
		...
		expect(postToolUse[1].hooks[0].command).toContain("CODEXSTORY_AGENT_NAME");
	});
	...
	test("SessionStart hook runs codexstory prime with agent name", async () => {
		const worktreePath = join(tempDir, "worktree");
		...
		expect(sessionStart.hooks[0].command).toContain("codexstory prime --agent prime-agent");
		expect(sessionStart.hooks[0].command).toContain("CODEXSTORY_AGENT_NAME");
	});
	...
	test("UserPromptSubmit hook runs mail check with agent name", async () => {
		...
		expect(userPrompt.hooks[0].command).toContain(
			"codexstory mail check --inject --agent mail-agent",
		);
		expect(userPrompt.hooks[0].command).toContain("CODEXSTORY_AGENT_NAME");
	});

	test("PreCompact hook runs codexstory prime with --compact flag", async () => {
		const worktreePath = join(tempDir, "worktree");
		...
		expect(preCompact.hooks[0].command).toContain(
			"codexstory prime --agent compact-agent --compact",
		);
		expect(preCompact.hooks[0].command).toContain("CODEXSTORY_AGENT_NAME");
	});

	test("PreToolUse hook pipes stdin to codexstory log with --stdin flag", async () => {
		const worktreePath = join(tempDir, "worktree");
		...
		expect(baseHook.hooks[0].command).toContain("codexstory log tool-start");
		expect(baseHook.hooks[0].command).toContain("stdin-agent");
		expect(baseHook.hooks[0].command).not.toContain("read -r INPUT");
	});

	test("PostToolUse hook pipes stdin to codexstory log with --stdin flag", async () => {
		const worktreePath = join(tempDir, "worktree");
		...
		expect(postToolUse.hooks[0].command).toContain("codexstory log tool-end");
	...
	test("PostToolUse hook includes mail check with debounce", async () => {
		...
		expect(postToolUse.hooks[1].command).toContain("codexstory mail check");
		...
		expect(postToolUse.hooks[1].command).toContain("CODEXSTORY_AGENT_NAME");
	});

	test("Stop hook pipes stdin to codexstory log with --stdin flag", async () => {
		const worktreePath = join(tempDir, "worktree");
		...
		expect(stop.hooks[0].command).toContain("codexstory log session-end");
	...
	test("Stop hook includes mulch learn command", async () => {
		...
		expect(stop.hooks[1].command).toContain("CODEXSTORY_AGENT_NAME");
	});
	...
	test("creates .codex directory even if worktree already exists", async () => {
	...
	test("throws AgentError when template is missing", async () => {
		...
		const exists = await Bun.file(join(worktreePath, ".codex", "settings.local.json")).exists();
		expect(exists).toBe(true);
	});
	...
	test("builder capability gets path boundary + Bash danger + Bash path boundary guards + native team tool blocks", async () => {
		...
		expect(writeGuards[0].hooks[0].command).toContain("CODEXSTORY_WORKTREE_PATH");
	...
	test("preserves user hooks alongside codexstory hooks", async () => {
		...
		expect(content).toContain("codexstory prime");
	});
	...
	test("codexstory hooks appear before user hooks per event type", async () => {
		...
		// User hook should be at the end (after all codexstory hooks)
		...
		const lastOverstoryIdx = preToolUse.reduce(
			(last: number, h: { hooks: Array<{ command: string }> }, i: number) => {
				if (
					h.hooks[0]?.command?.includes("codexstory") ||
					h.hooks[0]?.command?.includes("CODEXSTORY_")
				) {
					return i;
				}
				return last;
...
describe("isOverstoryHookEntry", () => {
	test("identifies entries with codexstory CLI commands", () => {
		expect(
			isOverstoryHookEntry({
				matcher: "",
				hooks: [{ type: "command", command: "codexstory prime --agent test" }],
			}),
		).toBe(true);
	});

	test("identifies entries with CODEXSTORY_ env var references", () => {
		expect(
			isOverstoryHookEntry({
				matcher: "Write",
				hooks: [
					{
						type: "command",
						command: '[ -z "$CODEXSTORY_AGENT_NAME" ] && exit 0; echo block',
					},
				],
			}),
		).toBe(true);
	...
	test("identifies entries with CODEXSTORY_WORKTREE_PATH", () => {
		expect(
			isOverstoryHookEntry({
				matcher: "Bash",
				hooks: [
					{
						type: "command",
						command: '[ -z "$CODEXSTORY_WORKTREE_PATH" ] && exit 0;',
					},
				],
			}),
		).toBe(true);
	...
	test("returns false for user hooks without codexstory references", () => {
		expect(
			isOverstoryHookEntry({
				matcher: "Bash",
				hooks: [{ type: "command", command: "echo user-custom-guard" }],
	...
	test("checks all hooks in the entry (any match = codexstory)", () => {
		expect(
			isOverstoryHookEntry({
				matcher: "",
				hooks: [
					{ type: "command", command: "echo user-thing" },
					{ type: "command", command: "codexstory mail check" },
				],
			}),
		).toBe(true);
	});
...
describe("getCapabilityGuards", () => {
	...
	test("builder gets Bash path boundary guard", () => {
		...
		expect(bashGuard?.hooks[0]?.command).toContain("CODEXSTORY_WORKTREE_PATH");
	...
	test("all capabilities get Task tool blocked", () => {
		for (const cap of [
			...
			expect(taskGuard?.hooks[0]?.command).toContain("codexstory sling");
		}
	});
	...
	test("block guard commands include env var guard prefix", () => {
		...
		for (const tool of ["Write", "Edit", "NotebookEdit"]) {
			...
			expect(guard?.hooks[0]?.command).toContain('[ -z "$CODEXSTORY_AGENT_NAME" ] && exit 0;');
		}
	});
	...
	test("native team tool block guards include env var guard prefix", () => {
		...
		const taskGuard = guards.find((g) => g.matcher === "Task");
		expect(taskGuard).toBeDefined();
		expect(taskGuard?.hooks[0]?.command).toContain('[ -z "$CODEXSTORY_AGENT_NAME" ] && exit 0;');
	});
...
describe("getDangerGuards", () => {
	...
	test("guard command includes agent name for branch validation", () => {
		...
		expect(command).toContain("codexstory/my-builder/");
	});
	...
	test("guard command includes env var guard prefix", () => {
		const guards = getDangerGuards("test-agent");
		const command = guards[0]?.hooks[0]?.command ?? "";
		expect(command).toContain('[ -z "$CODEXSTORY_AGENT_NAME" ] && exit 0;');
	});
	...
	test("all capabilities get Bash danger guards in deployed hooks", async () => {
		...
		const tempDir = await import("node:fs/promises").then((fs) =>
			fs.mkdtemp(join(require("node:os").tmpdir(), "codexstory-danger-test-")),
		);
		...
		try {
			for (const cap of capabilities) {
				...
				expect(bashGuard).toBeDefined();
				expect(bashGuard.hooks[0].command).toContain(`codexstory/${cap}-agent/`);
			}
		} finally {
	...
	test("guard ordering: path boundary → danger → capability in scout", async () => {
		const tempDir = await import("node:fs/promises").then((fs) =>
			fs.mkdtemp(join(require("node:os").tmpdir(), "codexstory-order-test-")),
		);
		...
		try {
			...
			const pathBoundaryWriteIdx = preToolUse.findIndex(
				(h: { matcher: string; hooks: Array<{ command: string }> }) =>
					h.matcher === "Write" && h.hooks[0]?.command?.includes("CODEXSTORY_WORKTREE_PATH"),
			);
...
describe("buildBashFileGuardScript", () => {
	...
	test("includes safe prefix whitelist checks", () => {
		const script = buildBashFileGuardScript("scout");
		expect(script).toContain("codexstory ");
	...
	test("includes env var guard prefix", () => {
		const script = buildBashFileGuardScript("scout");
		expect(script).toMatch(/^\[ -z "\$CODEXSTORY_AGENT_NAME" \] && exit 0;/);
	});
...
describe("structural enforcement integration", () => {
	...
	beforeEach(async () => {
		tempDir = await mkdtemp(join(tmpdir(), "codexstory-structural-test-"));
	});
	...
	test("non-implementation agents have more guards than implementation agents", async () => {
		...
		const scoutContent = await Bun.file(join(scoutPath, ".codex", "settings.local.json")).text();
		const builderContent = await Bun.file(
			join(builderPath, ".codex", "settings.local.json"),
		).text();
	...
	test("scout and reviewer have identical guard structures", async () => {
		...
		const reviewerContent = await Bun.file(
			join(reviewerPath, ".codex", "settings.local.json"),
		).text();
	...
	test("lead has same guard structure as scout/reviewer", async () => {
		...
		const leadContent = await Bun.file(join(leadPath, ".codex", "settings.local.json")).text();
		const scoutContent = await Bun.file(join(scoutPath, ".codex", "settings.local.json")).text();
	...
	test("builder and merger have identical guard structures", async () => {
		...
		const mergerContent = await Bun.file(join(mergerPath, ".codex", "settings.local.json")).text();
	...
	test("all deployed configs produce valid JSON", async () => {
		...
		for (const cap of capabilities) {
			...
			const content = await Bun.file(join(wt, ".codex", "settings.local.json")).text();
	...
	test("coordinator and supervisor have same guard structure", async () => {
		...
		const coordContent = await Bun.file(join(coordPath, ".codex", "settings.local.json")).text();
		const supContent = await Bun.file(join(supPath, ".codex", "settings.local.json")).text();
	...
	test("all template hooks include ENV_GUARD for project root isolation", async () => {
		...
		for (const hookType of [
			...
			expect(baseHook).toBeDefined();
			expect(baseHook?.hooks[0]?.command).toContain("CODEXSTORY_AGENT_NAME");
		}
		...
		expect(basePreToolUse?.hooks[0]?.command).toContain("CODEXSTORY_AGENT_NAME");
	});
			...
			for (const hook of entry.hooks) {
				if (hook.type === "command") {
					...
					if (
						...
						!hook.command.includes("CODEXSTORY_AGENT_NAME")
					) {
						continue;
					}
					expect(hook.command).toContain("CODEXSTORY_AGENT_NAME");
				}
			}
		}
	});

	test("all capabilities block Task tool for codexstory sling enforcement", async () => {
		const capabilities = [
			"scout",
			"reviewer",
			"lead",
	...
	test("all capabilities get path boundary guards in deployed hooks", async () => {
		...
		for (const cap of capabilities) {
			...
			// They use CODEXSTORY_WORKTREE_PATH env var
			const writeGuards = preToolUse.filter(
				(h: { matcher: string; hooks: Array<{ command: string }> }) =>
					h.matcher === "Write" && h.hooks[0]?.command?.includes("CODEXSTORY_WORKTREE_PATH"),
			);
			const editGuards = preToolUse.filter(
				(h: { matcher: string; hooks: Array<{ command: string }> }) =>
					h.matcher === "Edit" && h.hooks[0]?.command?.includes("CODEXSTORY_WORKTREE_PATH"),
			);
			const notebookGuards = preToolUse.filter(
				(h: { matcher: string; hooks: Array<{ command: string }> }) =>
					h.matcher === "NotebookEdit" && h.hooks[0]?.command?.includes("CODEXSTORY_WORKTREE_PATH"),
			);
...
describe("buildPathBoundaryGuardScript", () => {
	test("returns a string containing env var guard", () => {
		...
		expect(script).toContain('[ -z "$CODEXSTORY_AGENT_NAME" ] && exit 0;');
	});

	test("returns a string checking CODEXSTORY_WORKTREE_PATH", () => {
		const script = buildPathBoundaryGuardScript("file_path");
		expect(script).toContain('[ -z "$CODEXSTORY_WORKTREE_PATH" ] && exit 0;');
	});
	...
	test("allows paths inside the worktree", () => {
		const script = buildPathBoundaryGuardScript("file_path");
		expect(script).toContain('"$CODEXSTORY_WORKTREE_PATH"/*) exit 0');
	});
...
describe("getPathBoundaryGuards", () => {
	...
	test("all guards include CODEXSTORY_WORKTREE_PATH check", () => {
		const guards = getPathBoundaryGuards();
		for (const guard of guards) {
			expect(guard.hooks[0]?.command).toContain("CODEXSTORY_WORKTREE_PATH");
		}
	});
...
describe("buildBashPathBoundaryScript", () => {
	...
	test("checks CODEXSTORY_WORKTREE_PATH env var", () => {
		const script = buildBashPathBoundaryScript();
		expect(script).toContain('[ -z "$CODEXSTORY_WORKTREE_PATH" ] && exit 0;');
	});
	...
	test("validates paths against worktree boundary", () => {
		const script = buildBashPathBoundaryScript();
		expect(script).toContain('"$CODEXSTORY_WORKTREE_PATH"/*');
		expect(script).toContain('"$CODEXSTORY_WORKTREE_PATH")');
	});
...
describe("getBashPathBoundaryGuards", () => {
	...
	test("guard command checks CODEXSTORY_WORKTREE_PATH", () => {
		const guards = getBashPathBoundaryGuards();
		const command = guards[0]?.hooks[0]?.command ?? "";
		expect(command).toContain("CODEXSTORY_WORKTREE_PATH");
	});
...
describe("bash path boundary integration", () => {
	...
	beforeEach(async () => {
		tempDir = await mkdtemp(join(tmpdir(), "codexstory-bash-path-test-"));
	});
	...
	test("builder gets Bash path boundary guard in deployed hooks", async () => {
		...
		expect(pathGuard.hooks[0].command).toContain("CODEXSTORY_WORKTREE_PATH");
	});
	...
	test("deployed hooks include universal git push guard without ENV_GUARD", async () => {
		...
		const universalGuard = preToolUse.find(
			(h: { matcher: string; hooks: Array<{ command: string }> }) =>
				...
				!h.hooks[0]?.command?.includes("CODEXSTORY_AGENT_NAME"),
		);
