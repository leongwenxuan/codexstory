...
describe("lifecycle", () => {
	...
	beforeEach(async () => {
		agentsDir = await mkdtemp(join(tmpdir(), "codexstory-lifecycle-test-"));
	});
	...
	test("initiateHandoff creates checkpoint and handoff record", async () => {
		const handoff = await initiateHandoff({
			...
			beadId: "codexstory-xyz1",
			reason: "compaction",
			progressSummary: "Built the widget",
			pendingWork: "Tests remain",
			currentBranch: "codexstory/builder-1/codexstory-xyz1",
	...
	test("resumeFromHandoff returns pending handoff", async () => {
		await initiateHandoff({
			...
			beadId: "codexstory-abc2",
			reason: "crash",
			progressSummary: "Halfway done",
			pendingWork: "Finish implementation",
			currentBranch: "codexstory/builder-2/codexstory-abc2",
	...
	test("completeHandoff updates toSessionId and clears checkpoint", async () => {
		await initiateHandoff({
			...
			beadId: "codexstory-def3",
			reason: "manual",
			progressSummary: "Done with phase 1",
			pendingWork: "Phase 2",
			currentBranch: "codexstory/builder-3/codexstory-def3",
			filesModified: [],
			mulchDomains: [],
		});
	...
	test("multiple handoffs accumulate in handoffs.json", async () => {
		...
		await initiateHandoff({
			...
			beadId: "codexstory-ghi4",
			reason: "compaction",
			progressSummary: "First session work",
			pendingWork: "Continue",
			currentBranch: "codexstory/builder-4/codexstory-ghi4",
			filesModified: ["a.ts"],
			mulchDomains: [],
		});
	...
	test("resumeFromHandoff returns null when all handoffs are completed", async () => {
		await initiateHandoff({
			...
			beadId: "codexstory-jkl5",
			reason: "compaction",
			progressSummary: "Done",
			pendingWork: "Nothing",
			currentBranch: "codexstory/builder-5/codexstory-jkl5",
			filesModified: [],
			mulchDomains: [],
		});
