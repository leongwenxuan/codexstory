...
describe("checkpoint", () => {
	...
	beforeEach(async () => {
		agentsDir = await mkdtemp(join(tmpdir(), "codexstory-checkpoint-test-"));
	});
	...
	test("save and load a checkpoint", async () => {
		...
		expect(loaded?.beadId).toBe("codexstory-abc1");
		...
		expect(loaded?.currentBranch).toBe("codexstory/test-agent/codexstory-abc1");
