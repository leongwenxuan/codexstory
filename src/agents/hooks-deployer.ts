 ...
 * CODEXSTORY_WORKTREE_PATH env var set during tmux session creation
...
export function buildPathBoundaryGuardScript(filePathField: string): string {
	const script = [
		// Only enforce for codexstory agent sessions
		ENV_GUARD,
		// Skip if worktree path is not set (e.g., orchestrator)
		'[ -z "$CODEXSTORY_WORKTREE_PATH" ] && exit 0;',
		"read -r INPUT;",
		...
		'case "$FILE_PATH" in "$CODEXSTORY_WORKTREE_PATH"/*) exit 0 ;; "$CODEXSTORY_WORKTREE_PATH") exit 0 ;; esac;',
...
function buildBashGuardScript(agentName: string): string {
	...
	const script = [
		...
		'  echo \'{"decision":"block","reason":"git push is blocked — use codexstory merge to integrate changes, push manually when ready"}\';',
		"  exit 0;",
		"fi;",
		...
		`  if ! echo "$BRANCH" | grep -qE '^codexstory/${agentName}/'; then`,
		`    echo '{"decision":"block","reason":"Branch must follow codexstory/${agentName}/{bead-id} convention"}';`,
		"    exit 0;",
		"  fi;",
		"fi;",
	].join(" ");
 ...
 *
 * Uses CODEXSTORY_WORKTREE_PATH env var set during tmux session creation.
 */
...
export function getCapabilityGuards(capability: string): HookEntry[] {
	...
	const teamToolGuards = NATIVE_TEAM_TOOLS.map((tool) =>
		blockGuard(
			tool,
			`Overstory agents must use 'codexstory sling' for delegation — ${tool} is not allowed`,
		),
	);
...
/**
 * Check whether a hook entry is codexstory-managed.
 *
 * Overstory hook commands always reference either `codexstory` (CLI commands)
 * or `CODEXSTORY_` (env var guards like CODEXSTORY_AGENT_NAME, CODEXSTORY_WORKTREE_PATH).
 * User hooks will not contain these patterns.
 */
export function isOverstoryHookEntry(entry: HookEntry): boolean {
	return entry.hooks.some(
		(h) => h.command.includes("codexstory") || h.command.includes("CODEXSTORY_"),
	);
}

/**
 * Deploy hooks config to an agent's worktree as `.codex/settings.local.json`.
 *
 ...
 * Stale codexstory hook entries are stripped and replaced with the new set.
...
export async function deployHooks(
	...
	// Partition existing hooks: keep user entries, discard stale codexstory entries
	...
	// Merge: codexstory hooks first (security guards must run first), then user hooks
