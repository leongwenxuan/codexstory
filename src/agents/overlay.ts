...
function formatQualityGates(config: OverlayConfig): string {
	if (READ_ONLY_CAPABILITIES.has(config.capability)) {
		return [
			...
			`3. **Send results:** \`codexstory mail send --to ${config.parentAgent ?? "coordinator"} --subject "done" --body "Summary" --type result --agent ${config.agentName}\``,
			"",
	...
	return [
		...
		`${gateLines.length + 3}. **Signal completion:** send \`worker_done\` mail to ${config.parentAgent ?? "coordinator"}: \`codexstory mail send --to ${config.parentAgent ?? "coordinator"} --subject "Worker done: ${config.beadId}" --body "Quality gates passed." --type worker_done --agent ${config.agentName}\``,
		...
		"coordinator via `codexstory merge`.",
	].join("\n");
}
...
function formatConstraints(config: OverlayConfig): string {
	if (READ_ONLY_CAPABILITIES.has(config.capability)) {
		return [
			...
			`- Report completion via \`${config.trackerCli ?? "bd"} close\` AND \`codexstory mail send --type result\``,
...
function formatCanSpawn(config: OverlayConfig): string {
	...
	return [
		"You may spawn sub-workers using `codexstory sling`. Example:",
		"",
		"```bash",
		"codexstory sling <task-id> --capability builder --name <worker-name> \\",
...
/**
 * Generate a per-worker CODEXSTORY.md overlay from the template.
 *
 ...
 * `CODEXSTORY.md`, breaking the user's own Claude Code session (codexstory-uwg4).
 *
 * Uses deterministic path comparison instead of checking for `.codexstory/config.yaml`
 * because when dogfooding (running codexstory on its own repo), that file is tracked
 * in git and appears in every worktree checkout (codexstory-p4st).
 *
...
/**
 * Generate the overlay and write it to `{worktreePath}/CODEXSTORY.md`.
 *
...
export async function writeOverlay(
	...
	// The project root's CODEXSTORY.md belongs to the orchestrator/user.
	// Uses path comparison instead of file-existence heuristic to handle
	// dogfooding scenarios where .codexstory/config.yaml is tracked in git
	// and appears in every worktree checkout (codexstory-p4st).
	if (isCanonicalRoot(worktreePath, canonicalRoot)) {
		throw new AgentError(
			`Refusing to write overlay to canonical project root: ${worktreePath}. Agent overlays must target a worktree, not the orchestrator's root directory. This prevents overwriting the user's CODEXSTORY.md.`,
			{ agentName: config.agentName },
		);
	}

	const content = await generateOverlay(config);
	const outputPath = join(worktreePath, "CODEXSTORY.md");
