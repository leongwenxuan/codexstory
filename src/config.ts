...
export const DEFAULT_CONFIG: OverstoryConfig = {
	...
	agents: {
		manifestPath: ".codexstory/agent-manifest.json",
		baseDir: ".codexstory/agent-defs",
		maxConcurrent: 25,
...
function migrateDeprecatedWatchdogKeys(parsed: Record<string, unknown>): void {
	...
	process.stderr.write(
		"[codexstory] DEPRECATED: watchdog.tier1Enabled → use watchdog.tier0Enabled\n",
	);
	...
	if ("tier1IntervalMs" in wd) {
		...
		process.stderr.write(
			"[codexstory] DEPRECATED: watchdog.tier1IntervalMs → use watchdog.tier0IntervalMs\n",
		);
	}
	...
	if ("tier2Enabled" in wd) {
		...
		process.stderr.write(
			"[codexstory] DEPRECATED: watchdog.tier2Enabled → use watchdog.tier1Enabled\n",
		);
	}
}
...
function migrateDeprecatedTaskTrackerKeys(parsed: Record<string, unknown>): void {
	...
	if (parsed.beads !== undefined) {
		...
		delete parsed.beads;
		process.stderr.write(
			"[codexstory] DEPRECATED: beads: -> use taskTracker: { backend: beads, enabled: true }\n",
		);
		...
		delete parsed.seeds;
		process.stderr.write(
			"[codexstory] DEPRECATED: seeds: -> use taskTracker: { backend: seeds, enabled: true }\n",
		);
	}
}
	...
	for (const [role, model] of Object.entries(config.models)) {
		...
		if (model.includes("/")) {
			...
			if (toolHeavyRoles.includes(role)) {
				process.stderr.write(
					`[codexstory] WARNING: models.${role} uses non-Anthropic model '${model}'. Tool-use compatibility cannot be verified at config time.\n`,
				);
			}
		} else {
...
async function mergeLocalConfig(
	...
	const localPath = join(resolvedRoot, CODEXSTORY_DIR, CONFIG_LOCAL_FILENAME);
 ...
 * `.codexstory/worktrees/{name}/`), the passed directory won't contain
 * `.codexstory/config.yaml`. This function detects worktrees using
 ...
 * @returns The resolved project root containing `.codexstory/`
 */
export async function resolveProjectRoot(startDir: string): Promise<string> {
	...
	// (e.g., .codexstory/worktrees/{name}/), the worktree may contain
	// tracked copies of .codexstory/config.yaml. We must resolve to the
	...
	try {
		...
		if (exitCode === 0) {
			...
			if (mainRoot !== startDir && existsSync(join(mainRoot, CODEXSTORY_DIR, CONFIG_FILENAME))) {
				return mainRoot;
			}
		}
	} catch {
	...
	// Check if .codexstory/config.yaml exists at startDir.
	if (existsSync(join(startDir, CODEXSTORY_DIR, CONFIG_FILENAME))) {
		return startDir;
	}
...
/**
 * Load the codexstory configuration for a project.
 *
 * Reads `.codexstory/config.yaml` from the project root, parses it,
...
export async function loadConfig(projectRoot: string): Promise<OverstoryConfig> {
	...
	const configPath = join(resolvedRoot, CODEXSTORY_DIR, CONFIG_FILENAME);
