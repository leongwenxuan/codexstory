...
beforeEach(async () => {
	tempDir = await mkdtemp(join(tmpdir(), "codexstory-compat-test-"));
...
describe("openSessionStore", () => {
	...
	test("imports from sessions.json when sessions.db exists but is empty", async () => {
		...
		// a previous coordinator start that used the old code path (codexstory-036f).
...
describe("data integrity", () => {
	test("all fields from sessions.json are preserved in SQLite", async () => {
		const fullSession = makeJsonSession({
			...
			branchName: "codexstory/full-agent/task-42",
			beadId: "task-42",
			tmuxSession: "codexstory-full-agent",
			state: "stalled",
			pid: 99999,
		...
		expect(result?.branchName).toBe("codexstory/full-agent/task-42");
		expect(result?.beadId).toBe("task-42");
		expect(result?.tmuxSession).toBe("codexstory-full-agent");
