 ...
 *
 * Validates the "project-agnostic" promise by running codexstory init on a
 * fresh temp git repo (NOT the codexstory repo itself), then verifying all
...
describe("E2E: init→sling lifecycle on external project", () => {
	...
	test("loadConfig returns valid config pointing to temp dir", async () => {
		...
		expect(config.agents.baseDir).toBe(".codexstory/agent-defs");
	...
	test("manifest loads successfully with all 8 agents", async () => {
		...
		const manifestPath = join(tempDir, ".codexstory", "agent-manifest.json");
	...
	test("overlay generation works for external project", async () => {
		...
		const overlayConfig: OverlayConfig = {
			...
			specPath: null,
			branchName: "codexstory/test-agent/test-bead-001",
		...
		const worktreePath = join(tempDir, ".codexstory", "worktrees", "test-agent");
		...
		expect(content).toContain("codexstory/test-agent/test-bead-001");
	...
	test("full init→config→manifest→overlay pipeline succeeds", async () => {
		...
		const overlayConfig: OverlayConfig = {
			...
			specPath: join(tempDir, ".codexstory", "specs", "lifecycle-001.md"),
			branchName: "codexstory/lifecycle-builder/lifecycle-001",
			worktreePath: join(tempDir, ".codexstory", "worktrees", "lifecycle-builder"),
		...
		const worktreePath = join(tempDir, ".codexstory", "worktrees", "lifecycle-builder");
		const { mkdir } = await import("node:fs/promises");
		await mkdir(worktreePath, { recursive: true });

		await writeOverlay(worktreePath, overlayConfig, tempDir);

		const overlayContent = await Bun.file(join(worktreePath, ".codex", "CODEXSTORY.md")).text();
		...
		expect(overlayContent).toContain("codexstory/lifecycle-builder/lifecycle-001");
