/**
 * Tests for `codexstory costs` command.
 *
...
describe("costsCommand", () => {
	...
	describe("help flag", () => {
		test("--help shows help text", async () => {
			...
			expect(out).toContain("codexstory costs");
	...
	describe("JSON output mode", () => {
		test("outputs valid JSON array with sessions", async () => {
			const dbPath = join(tempDir, ".codexstory", "metrics.db");
	...
	describe("--self flag", () => {
		...
		test("--self shows orchestrator cost when transcript exists", async () => {
			...
			const projectDir = join(tempHome, ".codex", "projects", projectKey);
		...
		test("--self shows error when no transcript found", async () => {
			// No .codex directory — just set HOME to tempHome with nothing in it
			process.env.HOME = tempHome;
