/**
 * Tests for `codexstory trace` command.
 *
...
describe("traceCommand", () => {
	...
	describe("help flag", () => {
		test("--help shows help text", async () => {
			...
			expect(out).toContain("codexstory trace");
	...
	describe("argument parsing", () => {
		...
		test("target is extracted correctly when flags come first", async () => {
			...
			const dbPath = join(tempDir, ".codexstory", "events.db");
	...
	describe("target resolution", () => {
		...
		test("bead ID pattern is detected and resolved to agent name via SessionStore", async () => {
			...
			const sessDbPath = join(tempDir, ".codexstory", "sessions.db");
			...
			await traceCommand(["codexstory-rj1k", "--json"]);
			const out = output();
