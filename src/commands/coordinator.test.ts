/**
 * Tests for codexstory coordinator command.
 *
...
function makeCoordinatorSession(overrides: Partial<AgentSession> = {}): AgentSession {
	return {
		...
		branchName: "main",
		beadId: "",
		tmuxSession: "codexstory-test-project-coordinator",
		state: "working",
		pid: 99999,
		parentAgent: null,
		depth: 0,
 ...
 * Always injects fakes for all three to prevent real Bun.spawn(["codexstory", ...])
 * calls in tests (codexstory CLI is not available in CI). */
function makeDeps(
...
describe("coordinatorCommand help", () => {
	test("--help outputs help text", async () => {
		...
		expect(output).toContain("codexstory coordinator");
...
describe("startCoordinator", () => {
	test("writes session to sessions.json with correct fields", async () => {
		...
		expect(session?.tmuxSession).toBe("codexstory-test-project-coordinator");
		...
		expect(calls.createSession[0]?.name).toBe("codexstory-test-project-coordinator");
	...
	test("deploys hooks to project root .codex/settings.local.json", async () => {
		...
		// Verify .codex/settings.local.json was created at the project root
	...
	test("--json outputs JSON with expected fields", async () => {
		...
		expect(parsed.tmuxSession).toBe("codexstory-test-project-coordinator");
	...
	test("rejects duplicate when coordinator is already running", async () => {
		...
		const { deps } = makeDeps({ "codexstory-test-project-coordinator": true });
	...
	test("cleans up dead session and starts new one", async () => {
		...
		const { deps } = makeDeps({ "codexstory-test-project-coordinator": false });
...
describe("stopCoordinator", () => {
	test("marks session as completed after stopping", async () => {
		...
		const { deps, calls } = makeDeps({ "codexstory-test-project-coordinator": true });
		...
		expect(calls.killSession[0]?.name).toBe("codexstory-test-project-coordinator");
	});
	...
	test("handles already-dead tmux session gracefully", async () => {
		...
		const { deps, calls } = makeDeps({ "codexstory-test-project-coordinator": false });
...
describe("statusCoordinator", () => {
	...
	test("shows running state when coordinator is alive", async () => {
		...
		expect(output).toContain("codexstory-test-project-coordinator");
	});
...
describe("buildCoordinatorBeacon", () => {
	...
	test("includes coordinator identity in header", () => {
		const beacon = buildCoordinatorBeacon();
		expect(beacon).toContain("[CODEXSTORY] coordinator (coordinator)");
	});
	...
	test("includes startup instructions", () => {
		...
		expect(beacon).toContain("codexstory mail check --agent coordinator");
		expect(beacon).toContain("bd ready");
		expect(beacon).toContain("codexstory group status");
	});
...
describe("watchdog integration", () => {
	...
	describe("stopCoordinator watchdog cleanup", () => {
		test("always calls watchdog.stop() when stopping coordinator", async () => {
			...
			const { deps, watchdogCalls } = makeDeps(
				{ "codexstory-test-project-coordinator": true },
				{ stopSuccess: true },
			);
	...
	describe("statusCoordinator watchdog state", () => {
		test("includes watchdogRunning in JSON output when coordinator is running", async () => {
			const session = makeCoordinatorSession({ state: "working" });
			saveSessionsToDb([session]);
			const { deps } = makeDeps({ "codexstory-test-project-coordinator": true }, { running: true });
		...
		test("includes watchdogRunning:false in JSON output when watchdog is not running", async () => {
			const session = makeCoordinatorSession({ state: "working" });
			saveSessionsToDb([session]);
			const { deps } = makeDeps({ "codexstory-test-project-coordinator": true }, { running: false });
...
describe("monitor integration", () => {
	...
	describe("stopCoordinator monitor cleanup", () => {
		...
		test("--json output includes monitorStopped:true when monitor was running", async () => {
			const session = makeCoordinatorSession({ state: "working" });
			saveSessionsToDb([session]);
			const { deps } = makeDeps({ "codexstory-test-project-coordinator": true }, undefined, {
				stopSuccess: true,
			});
