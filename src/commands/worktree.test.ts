...
/**
 * Tests for `codexstory worktree` command.
 *
...
describe("worktreeCommand", () => {
	...
	beforeEach(async () => {
		...
		// Create temp git repo with .codexstory/config.yaml structure
		tempDir = await createTempGitRepo();
		// Normalize tempDir to resolve macOS /var -> /private/var symlink
		tempDir = realpathSync(tempDir);
		const overstoryDir = join(tempDir, ".codexstory");
	...
	function makeSession(overrides: Partial<AgentSession> = {}): AgentSession {
		return {
			...
			capability: "builder",
			worktreePath: join(tempDir, ".codexstory", "worktrees", "test-agent"),
			branchName: "codexstory/test-agent/task-1",
			beadId: "task-1",
			tmuxSession: "codexstory-test-agent-fake", // FAKE tmux session name
			state: "working",
			pid: 12345,
			parentAgent: null,
			depth: 0,
	...
	function writeSessionsToStore(sessions: AgentSession[]): void {
		const dbPath = join(tempDir, ".codexstory", "sessions.db");
	...
	describe("help flags", () => {
		test("--help shows help text", async () => {
			...
			expect(out).toContain("codexstory worktree");
	...
	describe("worktree list", () => {
		test("no codexstory worktrees returns empty message", async () => {
		...
		test("with codexstory worktrees lists them with agent info", async () => {
			// Create a real git worktree with codexstory/ prefix branch
			const worktreesDir = join(tempDir, ".codexstory", "worktrees");
			...
			expect(out).toContain("codexstory/test-agent/task-1");
		...
		test("--json flag outputs valid JSON array", async () => {
			...
			expect(parsed[0]?.branch).toBe("codexstory/test-agent/task-1");
		...
		test("worktrees without sessions show unknown state", async () => {
			...
			await runGitInDir(tempDir, [
				...
				worktreePath,
				"-b",
				"codexstory/orphan-agent/task-2",
			]);

			await worktreeCommand(["list"]);
			const out = output();

			expect(out).toContain("codexstory/orphan-agent/task-2");
	...
	describe("worktree clean", () => {
		...
		test("with completed agent worktree removes it and reports count", async () => {
			...
			await runGitInDir(tempDir, [
				...
				worktreePath,
				"-b",
				"codexstory/completed-agent/task-done",
			]);
			...
			writeSessionsToStore([
				{
					...
					capability: "builder",
					worktreePath,
					branchName: "codexstory/completed-agent/task-done",
					beadId: "task-done",
					tmuxSession: "codexstory-completed-agent",
					state: "completed",
					pid: 12345,
					parentAgent: null,
					depth: 0,
			...
			expect(out).toContain("🗑️  Removed: codexstory/completed-agent/task-done");
			...
			const branchListProc = Bun.spawn(["git", "branch", "--list", "codexstory/completed-agent/*"], {
				cwd: tempDir,
				stdout: "pipe",
			});
		...
		test("--json flag returns JSON with cleaned/failed/pruned arrays", async () => {
			...
			writeSessionsToStore([
				{
					...
					capability: "builder",
					worktreePath,
					branchName: "codexstory/done-agent/task-x",
					beadId: "task-x",
					tmuxSession: "codexstory-done-agent",
					state: "completed",
					pid: 12345,
					parentAgent: null,
					depth: 0,
			...
			expect(parsed.cleaned).toEqual(["codexstory/done-agent/task-x"]);
		...
		test("zombie sessions whose worktree paths no longer exist get pruned from sessions.db", async () => {
			...
			const nonExistentPath = join(tempDir, ".codexstory", "worktrees", "ghost-agent");
			writeSessionsToStore([
				{
					id: "session-ghost",
					...
					branchName: "codexstory/ghost-agent/task-ghost",
					beadId: "task-ghost",
					tmuxSession: "codexstory-ghost-agent",
					state: "zombie",
					pid: null,
					parentAgent: null,
					depth: 0,
		...
		test("stalled agents are cleaned like working agents (not by default)", async () => {
			...
			await runGitInDir(tempDir, [
				...
				worktreePath,
				"-b",
				"codexstory/stalled-agent/task-stuck",
			]);
			...
			writeSessionsToStore([
				{
					...
					capability: "builder",
					worktreePath,
					branchName: "codexstory/stalled-agent/task-stuck",
					beadId: "task-stuck",
		...
		test("--completed flag only cleans completed agents", async () => {
			...
			writeSessionsToStore([
				makeSession({
					...
					branchName: "codexstory/completed-agent/task-done",
					beadId: "task-done",
					tmuxSession: "codexstory-completed-agent-fake",
					state: "completed",
				}),
				makeSession({
					id: "session-2",
					...
					branchName: "codexstory/working-agent/task-wip",
					beadId: "task-wip",
					tmuxSession: "codexstory-working-agent-fake",
					state: "working",
					pid: 12346,
				}),
			]);
		...
		test("multiple completed worktrees reports correct count", async () => {
			...
			await runGitInDir(tempDir, ["worktree", "add", path1, "-b", "codexstory/agent-1/task-1"]);

			const path2 = join(worktreesDir, "agent-2");
			await runGitInDir(tempDir, ["worktree", "add", path2, "-b", "codexstory/agent-2/task-2"]);
			...
			writeSessionsToStore([
				{
					...
					capability: "builder",
					worktreePath: path1,
					branchName: "codexstory/agent-1/task-1",
					beadId: "task-1",
					tmuxSession: "codexstory-agent-1",
					state: "completed",
					pid: 12345,
					parentAgent: null,
					depth: 0,
				...
				},
				{
					...
					capability: "builder",
					worktreePath: path2,
					branchName: "codexstory/agent-2/task-2",
					beadId: "task-2",
					tmuxSession: "codexstory-agent-2",
					state: "completed",
					pid: 12346,
					parentAgent: null,
					depth: 0,
		...
		test("without --force, skips worktrees with unmerged branches and prints warning", async () => {
			...
			writeSessionsToStore([
				makeSession({
					...
					agentName: "unmerged-agent",
					worktreePath: wtPath,
					branchName: "codexstory/unmerged-agent/task-unmerged",
					beadId: "task-unmerged",
					state: "completed",
				}),
			]);
			...
			expect(out).toContain("codexstory/unmerged-agent/task-unmerged");
		...
		test("with --force, deletes worktrees with unmerged branches", async () => {
			...
			writeSessionsToStore([
				makeSession({
					...
					agentName: "unmerged-agent",
					worktreePath: wtPath,
					branchName: "codexstory/unmerged-agent/task-force",
					beadId: "task-force",
					state: "completed",
				}),
			]);
			...
			expect(out).toContain("🗑️  Removed: codexstory/unmerged-agent/task-force");
		});
		...
		test("--json output includes skipped array for unmerged branches", async () => {
			...
			writeSessionsToStore([
				makeSession({
					...
					agentName: "unmerged-json-agent",
					worktreePath: wtPath,
					branchName: "codexstory/unmerged-json-agent/task-json",
					beadId: "task-json",
					state: "completed",
				}),
			]);
			...
			expect(parsed.skipped).toEqual(["codexstory/unmerged-json-agent/task-json"]);
		});
	});
});
