/**
 * CLI command: codexstory sling <task-id>
 *
 ...
 * 7. Create worktree
 * 8. Generate + write overlay CODEXSTORY.md
 * 9. Deploy hooks config
 * 10. Claim beads issue
 * 11. Create agent identity
 * 12. Create tmux session running codex
 ...
 *
 * The beacon is the first user message sent to a Codex agent via
 * tmux send-keys. It provides identity context and a numbered startup
 * protocol so the agent knows exactly what to do on boot.
 *
 * Format:
 *   [CODEXSTORY] <agent-name> (<capability>) <ISO timestamp> task:<bead-id>
 *   Depth: <n> | Parent: <parent-name|none>
 *   Startup protocol:
 *   1. Read your assignment in CODEXSTORY.md
 *   2. Load expertise: mulch prime
 *   3. Check mail: codexstory mail check --agent <name>
...
export function buildBeacon(opts: BeaconOptions): string {
	...
	const parts = [
		`[CODEXSTORY] ${opts.agentName} (${opts.capability}) ${timestamp} task:${opts.taskId}`,
		`Depth: ${opts.depth} | Parent: ${parent}`,
		`Startup: read CODEXSTORY.md, run mulch prime, check mail (codexstory mail check --agent ${opts.agentName}), then begin task ${opts.taskId}`,
	];
	return parts.join(" — ");
}
...
/**
 * Entry point for `codexstory sling <task-id> [flags]`.
 *
 * Flags:
 ...
 */
const SLING_HELP = `codexstory sling — Spawn a worker agent

Usage: codexstory sling <task-id> [flags]
...
export async function slingCommand(args: string[]): Promise<void> {
	...
	// so agents in worktrees can access it (worktrees don't have .codexstory/)
	...
	try {
		...
		try {
			...
			// Clean up the orphaned worktree created in step 7 (codexstory-p4st)
			try {
		...
		const pid = await createSession(tmuxSessionName, worktreePath, codexCmd, {
			...
			CODEXSTORY_WORKTREE_PATH: worktreePath,
		});
		...
		// leaving the agent stuck in "booting" (codexstory-036f).
		...
		// Codex's TUI may consume early Enters during late initialization
		// (codexstory-yhv6). An Enter on an empty input line is harmless.
