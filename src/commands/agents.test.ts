...
describe("extractFileScope", () => {
	...
	it("should return empty array when 'No file scope restrictions'", async () => {
		const overlayPath = join(tempDir, ".codex", "CODEXSTORY.md");
...
describe("discoverAgents", () => {
	...
	it("should return active agents", async () => {
		...
		const session: AgentSession = {
			...
			capability: "builder",
			worktreePath: join(tempDir, ".codexstory", "worktrees", "builder-test"),
			branchName: "codexstory/builder-test/task-123",
			beadId: "task-123",
	...
	it("should filter by capability", async () => {
		...
		const scout: AgentSession = {
			...
			capability: "scout",
			worktreePath: join(tempDir, ".codexstory", "worktrees", "scout-test"),
			branchName: "codexstory/scout-test/task-456",
			beadId: "task-456",
			tmuxSession: "codexstory-test-scout",
			state: "working",
			pid: 12346,
			parentAgent: null,
			depth: 0,
	...
	it("should includeAll returns completed agents too", async () => {
		...
		const working: AgentSession = {
			...
			capability: "builder",
			worktreePath: join(tempDir, ".codexstory", "worktrees", "builder-working"),
			branchName: "codexstory/builder-working/task-123",
			beadId: "task-123",
			tmuxSession: "codexstory-test-working",
			state: "working",
			pid: 12345,
			parentAgent: null,
			depth: 0,
		...
		const completed: AgentSession = {
			...
			capability: "builder",
			worktreePath: join(tempDir, ".codexstory", "worktrees", "builder-completed"),
			branchName: "codexstory/builder-completed/task-456",
			beadId: "task-456",
			tmuxSession: "codexstory-test-completed",
			state: "completed",
			pid: null,
			parentAgent: null,
			depth: 0,
...
`;
	...
	it("should show help with --help flag", async () => {
		...
		expect(stdoutBuffer).toContain("codexstory agents");
