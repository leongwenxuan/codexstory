...
/**
 * Tests for `codexstory watch` command.
 *
...
describe("watchCommand", () => {
	...
	test("--help flag shows help text with key info", async () => {
		...
		expect(out).toContain("codexstory watch");
	...
	test("background mode: already running detection", async () => {
		...
		const pidFilePath = join(tempDir, ".codexstory", "watchdog.pid");
	...
	test("background mode: stale PID cleanup", async () => {
		...
		// The spawn will fail because there's no real codexstory binary in test env,
