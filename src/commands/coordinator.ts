/**
 * CLI command: codexstory coordinator start|stop|status
 *
 ...
 * and dispatches agents via codexstory sling.
 *
 ...
 * - Has no overlay CODEXSTORY.md (context comes via mail + beads + checkpoints)
 ...
 * Includes the project name to prevent cross-project collisions (codexstory-pcef).
 */
...
async function readWatchdogPid(projectRoot: string): Promise<number | null> {
	const pidFilePath = join(projectRoot, ".codexstory", "watchdog.pid");
 ...
 * Starts/stops the watchdog daemon via `codexstory watch --background`.
 */
function createDefaultWatchdog(projectRoot: string): NonNullable<CoordinatorDeps["_watchdog"]> {
	return {
		async start(): Promise<{ pid: number } | null> {
			...
			const proc = Bun.spawn(["codexstory", "watch", "--background"], {
				cwd: projectRoot,
				stdout: "pipe",
				stderr: "pipe",
			});
 ...
 * Starts/stops the monitor agent via `codexstory monitor start/stop`.
 */
function createDefaultMonitor(projectRoot: string): NonNullable<CoordinatorDeps["_monitor"]> {
	return {
		async start(): Promise<{ pid: number } | null> {
			const proc = Bun.spawn(["codexstory", "monitor", "start", "--no-attach", "--json"], {
				cwd: projectRoot,
				stdout: "pipe",
				stderr: "pipe",
			});
		...
		async stop(): Promise<boolean> {
			const proc = Bun.spawn(["codexstory", "monitor", "stop", "--json"], {
				cwd: projectRoot,
				stdout: "pipe",
				stderr: "pipe",
			});
		...
		async isRunning(): Promise<boolean> {
			const proc = Bun.spawn(["codexstory", "monitor", "status", "--json"], {
				cwd: projectRoot,
				stdout: "pipe",
				stderr: "pipe",
			});
 ...
 * via tmux send-keys after Codex initializes.
 *
 ...
 * 4. Deploy hooks to project root's .codex/settings.local.json
 * 5. Spawn tmux session at project root with Codex
...
async function startCoordinator(args: string[], deps: CoordinatorDeps = {}): Promise<void> {
	...
	try {
		...
		// leaving the coordinator stuck in "booting" (codexstory-036f).
...
const COORDINATOR_HELP = `codexstory coordinator — Manage the persistent coordinator agent

Usage: codexstory coordinator <subcommand> [flags]

Subcommands:
  start                    Start the coordinator (spawns Codex at project root)
...
The coordinator runs at the project root and orchestrates work by:
  ...
  - Dispatching agents via codexstory sling
  - Tracking batches via task groups
  - Handling escalations from agents and watchdog`;

/**
 * Entry point for `codexstory coordinator <subcommand>`.
 *
	...
	switch (subcommand) {
		...
		default:
			throw new ValidationError(
				`Unknown coordinator subcommand: ${subcommand}. Run 'codexstory coordinator --help' for usage.`,
