...
describe("logsCommand", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = join(
			tmpdir(),
			`codexstory-logs-test-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
		);
	...
	async function createConfig(): Promise<void> {
		const overstoryDir = join(tmpDir, ".codexstory");
...
`;
	...
	async function createLogFile(
		...
		const logsDir = join(tmpDir, ".codexstory", "logs", agentName, sessionTimestamp);
	...
	test("shows help text", async () => {
		...
		expect(output).toContain("codexstory logs");
	...
	test("handles malformed NDJSON lines gracefully", async () => {
		...
		const logsDir = join(tmpDir, ".codexstory", "logs", "agent-a", "2026-01-01T00-00-00-000Z");
		await mkdir(logsDir, { recursive: true });
