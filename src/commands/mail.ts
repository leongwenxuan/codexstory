/**
 * CLI command: codexstory mail send/check/list/read/reply
 *
...
function openStore(cwd: string) {
	const dbPath = join(cwd, ".codexstory", "mail.db");
...
function pendingNudgeDir(cwd: string): string {
	return join(cwd, ".codexstory", "pending-nudges");
}
 ...
 *
 * Creates `.codexstory/pending-nudges/{agent}.json` so that the next
...
function mailCheckStatePath(cwd: string): string {
	return join(cwd, ".codexstory", "mail-check-state.json");
}
...
async function handleSend(args: string[], cwd: string): Promise<void> {
	...
	if (isGroupAddress(to)) {
		const overstoryDir = join(cwd, ".codexstory");
		...
		try {
			...
			try {
				...
				for (const recipient of recipients) {
					...
					try {
						const eventsDbPath = join(cwd, ".codexstory", "events.db");
						const eventStore = createEventStore(eventsDbPath);
						try {
							let runId: string | null = null;
							const runIdPath = join(cwd, ".codexstory", "current-run.txt");
	...
	try {
		...
		// causing SIGKILL (exit 137) and "request interrupted" errors (codexstory-ii1o).
...
/**
 * Entry point for `codexstory mail <subcommand> [args...]`.
 *
 * Subcommands: send, check, list, read, reply, purge.
 */
const MAIL_HELP = `codexstory mail — Agent messaging system

Usage: codexstory mail <subcommand> [args...]
...
export async function mailCommand(args: string[]): Promise<void> {
	...
	// resolve up to the main project root where .codexstory/mail.db lives.
