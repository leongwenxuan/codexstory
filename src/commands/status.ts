/**
 * CLI command: codexstory status [--json] [--watch]
 *
...
export async function gatherStatus(
	...
	try {
		...
		try {
			const mailDbPath = join(root, ".codexstory", "mail.db");
		...
		try {
			const queuePath = join(root, ".codexstory", "merge-queue.db");
		...
		try {
			const metricsDbPath = join(root, ".codexstory", "metrics.db");
		...
		if (verbose && sessions.length > 0) {
			verboseDetails = {};
			for (const session of sessions) {
				const logsDir = join(root, ".codexstory", "logs", session.agentName);
...
export function printStatus(data: StatusData): void {
	...
	// Worktrees
	const overstoryWts = data.worktrees.filter((wt) => wt.branch.startsWith("codexstory/"));
...
/**
 * Entry point for `codexstory status [--json] [--watch]`.
 */
const STATUS_HELP = `codexstory status — Show all active agents and project state

Usage: codexstory status [--json] [--verbose] [--agent <name>] [--all]
...
Options:
  ...
  --watch            (deprecated) Use 'codexstory dashboard' for live monitoring
...
export async function statusCommand(args: string[]): Promise<void> {
	...
	if (watch) {
		process.stderr.write(
			"⚠️  --watch is deprecated. Use 'codexstory dashboard' for live monitoring.\n\n",
		);
