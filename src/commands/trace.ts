/**
 * CLI command: codexstory trace <target> [--json] [--since <ts>] [--until <ts>] [--limit <n>]
 *
 ...
 * Bead IDs follow the pattern: word-alphanumeric (e.g., "codexstory-rj1k", "myproject-abc1").
 */
...
const TRACE_HELP = `codexstory trace -- Show chronological timeline for an agent or bead

Usage: codexstory trace <target> [options]
...
/**
 * Entry point for `codexstory trace <target> [--json] [--since] [--until] [--limit]`.
 */
export async function traceCommand(args: string[]): Promise<void> {
	...
	if (!target) {
		throw new ValidationError("Missing target. Usage: codexstory trace <agent-name|bead-id>", {
			field: "target",
		});
	}
