 ...
 *
 * Written by `codexstory prime` at SessionStart when the orchestrator
...
async function loadOrchestratorTmuxSession(projectRoot: string): Promise<string | null> {
	const regPath = join(projectRoot, ".codexstory", "orchestrator-tmux.json");
 ...
 * file written by `codexstory prime`.
 */
...
export async function nudgeAgent(
	...
	if (!tmuxSessionName) {
		...
		let debounced = false;
		if (!force) {
			const statePath = join(projectRoot, ".codexstory", "nudge-state.json");
...
/**
 * Entry point for `codexstory nudge <agent-name> [message]`.
 */
const NUDGE_HELP = `codexstory nudge — Send a text nudge to an agent

Usage: codexstory nudge <agent-name> [message]
...
export async function nudgeCommand(args: string[]): Promise<void> {
	...
	if (!agentName || agentName.trim().length === 0) {
		throw new ValidationError("Agent name is required: codexstory nudge <agent-name> [message]", {
			field: "agentName",
		});
	}
