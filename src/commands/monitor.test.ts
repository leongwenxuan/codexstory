...
describe("buildMonitorBeacon", () => {
	...
	test("contains startup instruction: codexstory status --json", () => {
		const beacon = buildMonitorBeacon();
		expect(beacon).toContain("codexstory status --json");
	});

	test("contains startup instruction: codexstory mail check --agent monitor", () => {
		const beacon = buildMonitorBeacon();
		expect(beacon).toContain("codexstory mail check --agent monitor");
	});
	...
	test("contains [CODEXSTORY] prefix", () => {
		const beacon = buildMonitorBeacon();
		expect(beacon).toContain("[CODEXSTORY]");
	});
...
describe("monitorCommand", () => {
	...
	test("--help prints help text containing 'codexstory monitor'", async () => {
		await monitorCommand(["--help"]);
		const output = stdoutWrites.join("");
		expect(output).toContain("codexstory monitor");
	});
