...
describe("buildSupervisorBeacon", () => {
	...
	test("contains startup instructions", () => {
		...
		expect(beacon).toContain("codexstory mail check --agent supervisor-1");
	...
	test("contains ISO timestamp", () => {
		...
		// Extract timestamp from beacon (format: [CODEXSTORY] {name} (supervisor) {timestamp} task:{beadId})
...
describe("supervisorCommand", () => {
	test("--help prints help containing required keywords", async () => {
		...
		try {
			...
			expect(output).toContain("codexstory supervisor");
