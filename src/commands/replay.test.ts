/**
 * Tests for `codexstory replay` command.
 *
...
describe("replayCommand", () => {
	...
	describe("help flag", () => {
		test("--help shows help text", async () => {
			...
			expect(out).toContain("codexstory replay");
	...
	describe("default behavior", () => {
		test("uses current-run.txt when it exists", async () => {
			...
			await Bun.write(join(tempDir, ".codexstory", "current-run.txt"), "run-from-file\n");
		...
		test("falls back to timeline when current-run.txt is empty", async () => {
			...
			await Bun.write(join(tempDir, ".codexstory", "current-run.txt"), "");
