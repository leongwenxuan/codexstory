/**
 * CLI command: codexstory watch [--interval <ms>] [--background]
 *
...
/**
 * Resolve the path to the codexstory binary for re-launching.
 * Uses `which codexstory` first, then falls back to process.argv.
 */
async function resolveOverstoryBin(): Promise<string> {
	...
	throw new OverstoryError(
		"Cannot resolve codexstory binary path for background launch",
		"WATCH_ERROR",
	);
}

/**
 * Entry point for `codexstory watch [--interval <ms>] [--background]`.
 */
const WATCH_HELP = `codexstory watch — Start Tier 0 mechanical watchdog daemon

Usage: codexstory watch [--interval <ms>] [--background]
...
export async function watchCommand(args: string[]): Promise<void> {
	...
	const pidFilePath = join(config.project.root, ".codexstory", "watchdog.pid");
	...
	if (background) {
		...
		// Resolve the codexstory binary path
		const overstoryBin = await resolveOverstoryBin();

		// Spawn a detached background process running `codexstory watch` (without --background)
