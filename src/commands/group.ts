/**
 * CLI command: codexstory group create|status|add|remove|list
 *
 ...
 *
 * Storage: `.codexstory/groups.json` (array of TaskGroup objects).
 */
...
function groupsPath(projectRoot: string): string {
	return join(projectRoot, ".codexstory", "groups.json");
}

/**
 * Load groups from .codexstory/groups.json.
 */
...
/**
 * Save groups to .codexstory/groups.json.
 */
...
async function getGroupProgress(
	...
	if (completed === total && total > 0 && group.status === "active") {
		...
		try {
			const mailDbPath = join(projectRoot, ".codexstory", "mail.db");
...
const GROUP_HELP = `codexstory group -- Manage task groups for batch coordination

Usage: codexstory group <subcommand> [args...]
...
/**
 * Entry point for `codexstory group <subcommand>`.
 */
	...
	switch (subcommand) {
		case "create": {
			...
			if (!name || name.trim().length === 0) {
				throw new ValidationError(
					"Group name is required: codexstory group create '<name>' <id1> [id2...]",
					{ field: "name" },
				);
			}
			...
			if (issueIds.length === 0) {
				throw new ValidationError(
					"At least one issue ID is required: codexstory group create '<name>' <id1> [id2...]",
					{ field: "issueIds" },
				);
			}
		...
		case "add": {
			...
			if (!groupId || groupId.trim().length === 0) {
				throw new ValidationError(
					"Group ID is required: codexstory group add <group-id> <id1> [id2...]",
					{ field: "groupId" },
				);
			}
			const issueIds = positional.slice(1);
			if (issueIds.length === 0) {
				throw new ValidationError(
					"At least one issue ID is required: codexstory group add <group-id> <id1> [id2...]",
					{ field: "issueIds" },
				);
			}
		...
		case "remove": {
			...
			if (!groupId || groupId.trim().length === 0) {
				throw new ValidationError(
					"Group ID is required: codexstory group remove <group-id> <id1> [id2...]",
					{ field: "groupId" },
				);
			}
			const issueIds = positional.slice(1);
			if (issueIds.length === 0) {
				throw new ValidationError(
					"At least one issue ID is required: codexstory group remove <group-id> <id1> [id2...]",
					{ field: "issueIds" },
				);
			}
		...
		default:
			throw new ValidationError(
				`Unknown group subcommand: ${subcommand}. Run 'codexstory group --help' for usage.`,
