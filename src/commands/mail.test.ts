...
describe("mailCommand", () => {
	...
	beforeEach(async () => {
		tempDir = await mkdtemp(join(tmpdir(), "codexstory-mail-cmd-test-"));
		await mkdir(join(tempDir, ".codexstory"), { recursive: true });

		// Seed some messages via the store directly
		const store = createMailStore(join(tempDir, ".codexstory", "mail.db"));
		...
		// Change cwd to temp dir so the command finds .codexstory/mail.db
		origCwd = process.cwd();
	...
	describe("reply", () => {
		test("reply to own sent message goes to original recipient", async () => {
			...
			const store2 = createMailStore(join(tempDir, ".codexstory", "mail.db"));
		...
		test("reply with flags before positional ID extracts correct ID", async () => {
			// Regression test for codexstory-6nq: flags before the positional ID
	...
	describe("read", () => {
		test("read with flags before positional ID extracts correct ID", async () => {
			// Regression test for codexstory-6nq: same fragile pattern existed in handleRead.
			const store = createMailStore(join(tempDir, ".codexstory", "mail.db"));
			const client = createMailClient(store);
			const msgs = client.list({ to: "builder-1" });
			const originalId = msgs[0]?.id;
	...
	describe("auto-nudge (pending nudge markers)", () => {
		test("urgent message writes pending nudge marker instead of tmux keys", async () => {
			...
			const markerPath = join(tempDir, ".codexstory", "pending-nudges", "builder-1.json");
		...
		test("high priority message writes pending nudge marker", async () => {
			...
			const markerPath = join(tempDir, ".codexstory", "pending-nudges", "scout-1.json");
			const file = Bun.file(markerPath);
			expect(await file.exists()).toBe(true);
		...
		test("worker_done type writes pending nudge marker regardless of priority", async () => {
			...
			const markerPath = join(tempDir, ".codexstory", "pending-nudges", "orchestrator.json");
			const file = Bun.file(markerPath);
			expect(await file.exists()).toBe(true);
		...
		test("normal priority non-protocol message does NOT write marker", async () => {
			...
			const nudgeDir = join(tempDir, ".codexstory", "pending-nudges");
			try {
	...
	describe("mail_sent event recording", () => {
		...
		test("mail send includes run_id when current-run.txt exists", async () => {
			...
			await Bun.write(join(tempDir, ".codexstory", "current-run.txt"), runId);
	...
	describe("mail check debounce", () => {
		...
		test("mail check debounce state persists across invocations", async () => {
			...
			const statePath = join(tempDir, ".codexstory", "mail-check-state.json");
	...
	describe("broadcast", () => {
			...
			const sessions = [
				{
					...
					branchName: "main",
					beadId: "bead-001",
					tmuxSession: "codexstory-test-orchestrator",
				...
				},
				{
					...
					beadId: "bead-002",
					tmuxSession: "codexstory-test-builder-1",
					state: "working" as const,
					pid: 12346,
				...
				},
				{
					...
					beadId: "bead-003",
					tmuxSession: "codexstory-test-builder-2",
					state: "working" as const,
					pid: 12347,
					parentAgent: "orchestrator",
					depth: 1,
				...
				},
				{
					...
					branchName: "scout-1",
					beadId: "bead-004",
					tmuxSession: "codexstory-test-scout-1",
					state: "working" as const,
					pid: 12348,
					parentAgent: "orchestrator",
					depth: 1,
		...
		test("broadcast with urgent priority writes pending nudge for each recipient", async () => {
			...
			const nudgesDir = join(tempDir, ".codexstory", "pending-nudges");
		...
		async function seedSessions(
			...
			for (const [idx, session] of sessions.entries()) {
				sessionStore.upsert({
					...
					beadId: `bead-${idx}`,
					tmuxSession: `codexstory-test-${session.agentName}`,
					state: "working" as const,
					pid: 10000 + idx,
