/**
 * CLI command: codexstory merge
 *
 * Merges agent branches back to the canonical branch using
 * the merge queue and tiered conflict resolver.
 *
 * Usage:
 *   codexstory merge --branch <name>   Merge a specific branch
 *   codexstory merge --all             Merge all pending branches
 *   codexstory merge --dry-run         Check for conflicts without merging
 *   codexstory merge --json            Output results as JSON
 */
...
/**
 * Extract agent name from a branch following the codexstory naming convention.
 * Pattern: codexstory/{agentName}/{beadId}
 * Falls back to "unknown" if the pattern does not match.
 */
function parseAgentName(branchName: string): string {
	const parts = branchName.split("/");
	if (parts[0] === "codexstory" && parts[1] !== undefined) {
		return parts[1];
	}
	return "unknown";
}
...
/**
 * Extract bead ID from a branch following the codexstory naming convention.
 * Pattern: codexstory/{agentName}/{beadId}
 * Falls back to "unknown" if the pattern does not match.
 */
function parseBeadId(branchName: string): string {
	const parts = branchName.split("/");
	if (parts[0] === "codexstory" && parts[2] !== undefined) {
		return parts[2];
	}
	return "unknown";
}
...
/**
 * Entry point for `codexstory merge [flags]`.
 *
 * Flags:
 ...
 */
const MERGE_HELP = `codexstory merge — Merge agent branches into canonical

Usage: codexstory merge --branch <name> | --all [--into <branch>] [--dry-run] [--json]
...
export async function mergeCommand(args: string[]): Promise<void> {
	...
	if (!branchName && !all) {
		throw new ValidationError("Either --branch <name> or --all is required for codexstory merge", {
			field: "branch|all",
		});
	}
	...
	const queuePath = join(config.project.root, ".codexstory", "merge-queue.db");
