/**
 * CLI command: codexstory log <event> --agent <name> [--stdin]
 *
 * Called by Pre/PostToolUse and Stop hooks.
 * Events: tool-start, tool-end, session-end.
 * Writes to .codexstory/logs/{agent-name}/{session-timestamp}/.
 *
...
export async function autoRecordExpertise(params: {
	...
	let insightSummary = "";
	try {
		const eventsDbPath = join(params.projectRoot, ".codexstory", "events.db");
...
/**
 * Entry point for `codexstory log <event> --agent <name>`.
 */
const LOG_HELP = `codexstory log — Log a hook event

Usage: codexstory log <event> --agent <name> [--stdin]
