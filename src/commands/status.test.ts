...
/**
 * Tests for the --verbose flag in codexstory status.
 *
...
function makeAgent(overrides: Partial<AgentSession> = {}): AgentSession {
	return {
		...
		branchName: "codexstory/test-builder/task-1",
		beadId: "task-1",
		tmuxSession: "codexstory-test-builder",
		state: "working",
		pid: 12345,
		parentAgent: null,
		depth: 0,
...
function makeStatusData(overrides: Partial<StatusData> = {}): StatusData {
	return {
		agents: [makeAgent()],
		worktrees: [],
		tmuxSessions: [{ name: "codexstory-test-builder", pid: 12345 }],
		unreadMailCount: 0,
		mergeQueueCount: 0,
...
describe("printStatus", () => {
	...
	test("verbose: shows worktree path, logs dir, and mail timestamps", () => {
		const detail: VerboseAgentDetail = {
			worktreePath: "/tmp/worktrees/test-builder",
			logsDir: "/tmp/.codexstory/logs/test-builder",
		...
		expect(out).toContain("Logs:     /tmp/.codexstory/logs/test-builder");
	...
	test("verbose: zombie agents do not get verbose detail", () => {
		...
		const detail: VerboseAgentDetail = {
			...
			logsDir: "/tmp/.codexstory/logs/zombie-agent",
			lastMailSent: null,
			lastMailReceived: null,
			capability: "builder",
		};
	...
	test("verbose with multiple agents: each gets its own detail", () => {
		const agent1 = makeAgent({ agentName: "builder-1", tmuxSession: "codexstory-builder-1" });
		const agent2 = makeAgent({
			agentName: "scout-1",
			capability: "scout",
			tmuxSession: "codexstory-scout-1",
		});

		const data = makeStatusData({
			agents: [agent1, agent2],
			tmuxSessions: [
				{ name: "codexstory-builder-1", pid: 100 },
				{ name: "codexstory-scout-1", pid: 200 },
			],
			verboseDetails: {
				"builder-1": {
	...
	test("gatherStatus includes null-runId sessions when run-scoped", async () => {
		...
		for (const session of [
			makeAgent({
				...
				runId: null,
				tmuxSession: "codexstory-fake-coordinator",
			}),
			makeAgent({
				id: "sess-002",
				agentName: "builder-1",
				capability: "builder",
				runId: "run-001",
				tmuxSession: "codexstory-fake-builder-1",
			}),
			makeAgent({
				id: "sess-003",
				agentName: "builder-2",
				capability: "builder",
				runId: "run-002",
				tmuxSession: "codexstory-fake-builder-2",
			}),
		] as AgentSession[]) {
...
describe("--watch deprecation", () => {
	test("help text marks --watch as deprecated", async () => {
		...
		expect(out).toContain("codexstory dashboard");
	});
	...
	test("--watch writes deprecation notice to stderr", async () => {
		...
		// statusCommand with --watch will fail at loadConfig (no .codexstory/)
		...
		try {
			...
			// Expected: loadConfig fails without .codexstory/
		} finally {
		...
		expect(err).toContain("codexstory dashboard");
	});
});
