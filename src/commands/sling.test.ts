 ...
 * multiline tmux send-keys issues (codexstory-y2ob, codexstory-cczf).
 */
...
describe("buildBeacon", () => {
	...
	test("includes agent identity and task ID in header", () => {
		...
		expect(beacon).toContain("[CODEXSTORY] test-builder (builder) ");
		expect(beacon).toContain("task:codexstory-abc");
	});
	...
	test("includes startup instructions with agent name and task ID", () => {
		const opts = makeBeaconOpts({ agentName: "scout-1", taskId: "codexstory-xyz" });
		const beacon = buildBeacon(opts);

		expect(beacon).toContain("read CODEXSTORY.md");
		expect(beacon).toContain("mulch prime");
		expect(beacon).toContain("codexstory mail check --agent scout-1");
		expect(beacon).toContain("begin task codexstory-xyz");
	});

	test("uses agent name in mail check command", () => {
		const beacon = buildBeacon(makeBeaconOpts({ agentName: "reviewer-beta" }));

		expect(beacon).toContain("codexstory mail check --agent reviewer-beta");
	});
	...
	test("works with hierarchy depth > 0 and parent", () => {
		const beacon = buildBeacon(
			makeBeaconOpts({
				agentName: "worker-3",
				capability: "builder",
				taskId: "codexstory-deep",
				parentAgent: "lead-main",
				depth: 2,
			}),
		);

		expect(beacon).toContain("[CODEXSTORY] worker-3 (builder)");
		expect(beacon).toContain("task:codexstory-deep");
...
describe("checkBeadLock", () => {
	test("returns null when no sessions exist", () => {
		expect(checkBeadLock([], "codexstory-abc")).toBeNull();
	});

	test("returns null when no session matches the bead ID", () => {
		const sessions = [
			makeBeadSession("builder-1", "codexstory-xyz"),
			makeBeadSession("builder-2", "codexstory-def"),
		];

		expect(checkBeadLock(sessions, "codexstory-abc")).toBeNull();
	});

	test("returns the agent name when a session matches", () => {
		const sessions = [
			makeBeadSession("builder-1", "codexstory-abc"),
			makeBeadSession("builder-2", "codexstory-xyz"),
		];

		expect(checkBeadLock(sessions, "codexstory-abc")).toBe("builder-1");
	});
	...
	test("returns the first matching agent when multiple sessions match", () => {
		...
		const sessions = [
			makeBeadSession("builder-1", "codexstory-abc"),
			makeBeadSession("builder-2", "codexstory-abc"),
		];
...
describe("checkBeadLock parent bypass", () => {
	test("parent matching lock holder is allowed (returns lock holder name for caller to compare)", () => {
		...
		const sessions = [makeBeadSession("lead-alpha", "codexstory-abc")];
		// checkBeadLock still returns the holder — the caller (slingCommand) decides
		// whether to allow based on parentAgent match.
		expect(checkBeadLock(sessions, "codexstory-abc")).toBe("lead-alpha");
	});

	test("non-parent lock holder blocks spawn", () => {
		const sessions = [makeBeadSession("other-agent", "codexstory-abc")];
		const lockHolder = checkBeadLock(sessions, "codexstory-abc");
