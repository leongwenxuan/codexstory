...
import { initCommand, CODEXSTORY_GITIGNORE, CODEXSTORY_README } from "./init.ts";

/**
 * Tests for `codexstory init` -- agent definition deployment.
 *
...
describe("initCommand: agent-defs deployment", () => {
	...
	test("creates .codexstory/agent-defs/ with all 8 agent definition files", async () => {
		await initCommand([]);

		const agentDefsDir = join(tempDir, ".codexstory", "agent-defs");
	...
	test("copied files match source content", async () => {
		...
		for (const fileName of AGENT_DEF_FILES) {
			...
			const targetPath = join(tempDir, ".codexstory", "agent-defs", fileName);
	...
	test("--force reinit overwrites existing agent def files", async () => {
		...
		const tamperPath = join(tempDir, ".codexstory", "agent-defs", "scout.md");
...
describe("initCommand: .codexstory/.gitignore", () => {
	let tempDir: string;
	let originalCwd: string;
	let originalWrite: typeof process.stdout.write;
	...
	test("creates .codexstory/.gitignore with wildcard+whitelist model", async () => {
		await initCommand([]);
		...
		expect(content).toBe(CODEXSTORY_GITIGNORE);
	});
	...
	test("--force reinit overwrites stale .codexstory/.gitignore", async () => {
		// First init
		await initCommand([]);
		...
		expect(restored).toBe(CODEXSTORY_GITIGNORE);
...
describe("initCommand: .codexstory/README.md", () => {
	let tempDir: string;
	let originalCwd: string;
	let originalWrite: typeof process.stdout.write;
	...
	test("creates .codexstory/README.md with expected content", async () => {
		await initCommand([]);

		const readmePath = join(tempDir, ".codexstory", "README.md");
		const exists = await Bun.file(readmePath).exists();
		expect(exists).toBe(true);

		const content = await Bun.file(readmePath).text();
		expect(content).toBe(CODEXSTORY_README);
	});

	test("README.md is whitelisted in gitignore", () => {
		expect(CODEXSTORY_GITIGNORE).toContain("!README.md\n");
	});
	...
	test("--force reinit overwrites README.md", async () => {
		...
		expect(restored).toBe(CODEXSTORY_README);
	});
...
describe("initCommand: canonical branch detection", () => {
	...
	test("non-standard branch names are accepted as canonicalBranch", async () => {
		...
		const configPath = join(tempDir, ".codexstory", "config.yaml");
