/**
 * Shell completion generation for codexstory CLI.
 *
...
export const COMMANDS: readonly CommandDef[] = [
	...
	{
		name: "init",
		desc: "Initialize .codexstory/ in current project",
		flags: [
	...
	{
		name: "doctor",
		desc: "Run health checks on codexstory setup",
		flags: [
			{ name: "--json", desc: "JSON output" },
...
export function generateBash(): string {
	const lines: string[] = [
		"# Bash completion for codexstory",
		"# Source this file to enable completions:",
		"#   source <(codexstory --completions bash)",
		"",
		"_overstory() {",
	...
	lines.push("}");
	lines.push("");
	lines.push("complete -F _overstory codexstory");
...
export function generateZsh(): string {
	const lines: string[] = [
		"#compdef codexstory",
		"# Zsh completion for codexstory",
		"# Place this file in your fpath or source it:",
		"#   source <(codexstory --completions zsh)",
		"",
		"_overstory() {",
		"  local -a commands",
		"  commands=(",
...
export function generateFish(): string {
	const lines: string[] = [
		"# Fish completion for codexstory",
		"# Place this file in ~/.config/fish/completions/codexstory.fish or source it:",
		"#   codexstory --completions fish | source",
		"",
		"# Remove all existing completions for codexstory",
		"complete -c codexstory -e",
		"",
		"# Global options",
		"complete -c codexstory -l help -d 'Show help'",
		"complete -c codexstory -l version -d 'Show version'",
		"complete -c codexstory -l completions -d 'Generate shell completions' -xa 'bash zsh fish'",
		"",
	];
	...
	for (const cmd of COMMANDS) {
		...
		lines.push(`# ${cmd.desc}`);
		lines.push(
			`complete -c codexstory -f -n '__fish_use_subcommand' -a '${cmd.name}' -d '${cmd.desc}'`,
		);
		...
		if (cmd.subcommands && cmd.subcommands.length > 0) {
			...
			for (const subcmd of cmd.subcommands) {
				lines.push(
					`complete -c codexstory -f -n '__fish_seen_subcommand_from ${cmd.name}; and not __fish_seen_subcommand_from ${cmd.subcommands.map((s) => s.name).join(" ")}' -a '${subcmd.name}' -d '${subcmd.desc}'`,
				);
				...
				if (subcmd.flags && subcmd.flags.length > 0) {
					for (const flag of subcmd.flags) {
						...
						if (flag.values) {
							lines.push(
								`complete -c codexstory -f -n ${cond} -l '${flagName}' -d '${flag.desc}' -xa '${flag.values.join(" ")}'`,
							);
						} else if (flag.takesValue) {
							lines.push(`complete -c codexstory -n ${cond} -l '${flagName}' -d '${flag.desc}'`);
						} else {
							lines.push(`complete -c codexstory -f -n ${cond} -l '${flagName}' -d '${flag.desc}'`);
						}
					}
				}
			}
...
export function completionsCommand(args: string[]): void {
	...
	if (!shell) {
		...
		process.stderr.write("Usage: codexstory --completions <bash|zsh|fish>\n");
		process.exit(1);
	}
