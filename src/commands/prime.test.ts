...
/**
 * Tests for `codexstory prime` command.
 *
...
describe("primeCommand", () => {
	...
	describe("Help", () => {
		test("--help shows help text", async () => {
			...
			expect(out).toContain("codexstory prime");
	...
	describe("Agent priming (--agent <name>)", () => {
		...
		test("agent with identity.yaml shows identity details", async () => {
			// Write identity.yaml
			const agentDir = join(tempDir, ".codexstory", "agents", "my-builder");
			await Bun.write(
...
`,
		...
		test("agent with active session shows Activation section", async () => {
			...
			const sessions: AgentSession[] = [
				{
					...
					capability: "builder",
					worktreePath: join(tempDir, ".codexstory", "worktrees", "active-builder"),
					branchName: "codexstory/active-builder/task-001",
					beadId: "task-001",
					tmuxSession: "codexstory-active-builder",
					state: "working",
					pid: 12345,
					parentAgent: null,
					depth: 0,
			...
			await Bun.write(
				join(tempDir, ".codexstory", "sessions.json"),
			...
			const sessions: AgentSession[] = [
				{
					...
					capability: "builder",
					worktreePath: join(tempDir, ".codexstory", "worktrees", "completed-builder"),
					branchName: "codexstory/completed-builder/task-002",
					beadId: "task-002",
					tmuxSession: "codexstory-completed-builder",
					state: "completed",
					pid: null,
					parentAgent: null,
					depth: 0,
		...
		test("--compact with checkpoint.json shows Session Recovery", async () => {
			...
			const agentDir = join(tempDir, ".codexstory", "agents", "recovery-agent");
			await Bun.write(
				...
				`${JSON.stringify(
					{
						...
						currentBranch: "codexstory/recovery-agent/task-003",
...
`,
			...
			expect(out).toContain("Branch:** codexstory/recovery-agent/task-003");
		});

		test("--compact skips Expertise section", async () => {
			// Write identity with expertise
			const agentDir = join(tempDir, ".codexstory", "agents", "compact-agent");
			await Bun.write(
				join(agentDir, "identity.yaml"),
				`name: compact-agent
capability: builder
...
`,
	...
	describe("Session branch capture", () => {
		test("orchestrator prime writes session-branch.txt with current git branch", async () => {
			...
			try {
				const overstoryDir = join(gitRepoDir, ".codexstory");
...
`;
		...
		test("creates .codexstory/.gitignore if missing", async () => {
			// The beforeEach creates .codexstory/config.yaml but not .gitignore
			const gitignorePath = join(tempDir, ".codexstory", ".gitignore");
		...
		test("overwrites stale .codexstory/.gitignore with current template", async () => {
...
`;
		...
		test("does not overwrite .codexstory/.gitignore if already correct", async () => {
			// Write the correct CODEXSTORY_GITIGNORE content
			const gitignorePath = join(tempDir, ".codexstory", ".gitignore");
