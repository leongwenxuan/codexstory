/**
 * CLI command: codexstory supervisor start|stop|status
 *
...
export function buildSupervisorBeacon(opts: {
	...
	const parts = [
		`[CODEXSTORY] ${opts.name} (supervisor) ${timestamp} task:${opts.beadId}`,
		`Depth: ${opts.depth} | Parent: ${opts.parent} | Role: per-project supervisor`,
		`Startup: run mulch prime, check mail (codexstory mail check --agent ${opts.name}), read task (${cli} show ${opts.beadId}), then begin supervising`,
	];
	return parts.join(" — ");
}
 ...
 * 7. Spawn tmux session at project root with Codex
...
const SUPERVISOR_HELP = `codexstory supervisor — Manage per-project supervisor agents

Usage: codexstory supervisor <subcommand> [flags]

Subcommands:
  start                    Start a supervisor (spawns Codex at project root)
...
via codexstory sling and coordinate their work.`;

/**
 * Entry point for `codexstory supervisor <subcommand>`.
 */
	...
	switch (subcommand) {
		...
		default:
			throw new ValidationError(
				`Unknown supervisor subcommand: ${subcommand}. Run 'codexstory supervisor --help' for usage.`,
