...
/**
 * Tests for `codexstory log` command.
 *
...
describe("logCommand", () => {
	...
	beforeEach(async () => {
		...
		// Create temp dir with .codexstory/config.yaml structure
	...
	test("--help flag shows help text", async () => {
		...
		expect(out).toContain("codexstory log");
	...
	test("tool-start creates log directory structure", async () => {
		...
		const logsDir = join(tempDir, ".codexstory", "logs", "test-builder");
	...
	test("tool-start creates session directory and .current-session marker", async () => {
		...
		const logsDir = join(tempDir, ".codexstory", "logs", "test-scout");
	...
	test("tool-end uses the same session directory as tool-start", async () => {
		...
		const logsDir = join(tempDir, ".codexstory", "logs", "test-agent");
		const markerPath = join(logsDir, ".current-session");
	...
	test("tool-end writes to the same session directory", async () => {
		...
		const logsDir = join(tempDir, ".codexstory", "logs", "test-worker");
		const markerPath = join(logsDir, ".current-session");
		const sessionDir = (await Bun.file(markerPath).text()).trim();
	...
	test("session-end clears the .current-session marker", async () => {
		...
		const logsDir = join(tempDir, ".codexstory", "logs", "test-cleanup");
		const markerPath = join(logsDir, ".current-session");
	...
	test("session-end records metrics when agent session exists in sessions.db", async () => {
		// Create sessions.db with a test agent
		const sessionsDbPath = join(tempDir, ".codexstory", "sessions.db");
		const session: AgentSession = {
			id: "session-002",
		...
		const metricsDbPath = join(tempDir, ".codexstory", "metrics.db");
	...
	test("session-end does NOT transition coordinator to completed (persistent agent)", async () => {
		...
		const session: AgentSession = {
			...
			branchName: "main",
			beadId: "",
			tmuxSession: "codexstory-coordinator",
			state: "working",
			pid: 11111,
			parentAgent: null,
			depth: 0,
	...
	test("session-end does NOT transition monitor to completed (persistent agent)", async () => {
		...
		const session: AgentSession = {
			...
			branchName: "main",
			beadId: "",
			tmuxSession: "codexstory-monitor",
			state: "working",
			pid: 22222,
			parentAgent: null,
			depth: 0,
	...
	describe("session-end coordinator run completion", () => {
		test("session-end auto-completes the active run for coordinator agent", async () => {
			...
			const currentRunPath = join(tempDir, ".codexstory", "current-run.txt");
		...
		test("session-end does not fail when no active run for coordinator", async () => {
			...
			sessionStoreLocal.upsert({
				...
				branchName: "main",
				beadId: "",
				tmuxSession: "codexstory-coordinator-no-run",
				state: "working",
				pid: 11112,
				parentAgent: null,
				depth: 0,
		...
		test("session-end does not complete run for non-coordinator agents", async () => {
			...
			sessionStoreLocal.upsert({
				...
				tmuxSession: "codexstory-builder",
				state: "working",
				pid: 11113,
				parentAgent: null,
				depth: 2,
			...
			// Write current-run.txt
			await Bun.write(join(tempDir, ".codexstory", "current-run.txt"), "run-test-002");
			...
			const currentRunFile = Bun.file(join(tempDir, ".codexstory", "current-run.txt"));
		...
		test("session-end handles already-completed run gracefully", async () => {
			...
			sessionStoreLocal.upsert({
				...
				branchName: "main",
				beadId: "",
				tmuxSession: "codexstory-coordinator-completed",
				state: "working",
				pid: 11114,
				parentAgent: null,
				depth: 0,
			...
			// Write current-run.txt
			await Bun.write(join(tempDir, ".codexstory", "current-run.txt"), "run-test-003");
	...
	test("session-end writes pending-nudge marker for coordinator when lead completes", async () => {
		...
		const session: AgentSession = {
			...
			tmuxSession: "codexstory-lead-alpha",
			state: "working",
			pid: 33333,
			parentAgent: null,
			depth: 0,
		...
		const markerPath = join(tempDir, ".codexstory", "pending-nudges", "coordinator.json");
		const markerFile = Bun.file(markerPath);
		expect(await markerFile.exists()).toBe(true);
	...
	test("session-end does NOT write pending-nudge marker for non-lead agents", async () => {
		...
		const session: AgentSession = {
			...
			beadId: "bead-builder-001",
			tmuxSession: "codexstory-builder-beta",
			state: "working",
			pid: 44444,
			parentAgent: null,
			depth: 0,
	...
	test("tool-start defaults to unknown when --tool-name not provided", async () => {
		...
		const logsDir = join(tempDir, ".codexstory", "logs", "default-tool-agent");
		const markerPath = join(logsDir, ".current-session");
		const markerFile = Bun.file(markerPath);
	...
	test("tool-end defaults to unknown when --tool-name not provided", async () => {
		...
		const logsDir = join(tempDir, ".codexstory", "logs", "default-end-agent");
		const markerPath = join(logsDir, ".current-session");
		const sessionDir = (await Bun.file(markerPath).text()).trim();
		const eventsFile = Bun.file(join(sessionDir, "events.ndjson"));
		const eventsContent = await eventsFile.text();
	...
	test("session-end does not crash when mulch learn/record fails", async () => {
		...
		const session: AgentSession = {
			...
			tmuxSession: "codexstory-mulch-fail",
			state: "working",
			pid: 55555,
			parentAgent: "parent-agent",
			depth: 1,
	...
	test("session-end skips mulch auto-record for coordinator (persistent agent)", async () => {
		...
		const session: AgentSession = {
			...
			branchName: "main",
			beadId: "",
			tmuxSession: "codexstory-coordinator-mulch",
			state: "working",
			pid: 66666,
			parentAgent: null,
			depth: 0,
		...
		const mailDbPath = join(tempDir, ".codexstory", "mail.db");
	...
	test("autoRecordExpertise calls record for each suggested domain", async () => {
		...
		const mailDbPath = join(tempDir, ".codexstory", "auto-record-mail.db");
	...
	test("autoRecordExpertise sends mail with auto-recorded subject", async () => {
		...
		const mailDbPath = join(tempDir, ".codexstory", "auto-record-mail2.db");
	...
	test("autoRecordExpertise continues when individual record calls fail", async () => {
		...
		const mailDbPath = join(tempDir, ".codexstory", "auto-record-fail.db");
	...
	test("autoRecordExpertise returns empty when no domains suggested", async () => {
		...
		const { client, recordCalls } = createFakeMulchClient(learnResult);
		const mailDbPath = join(tempDir, ".codexstory", "auto-record-empty.db");
	...
	test("autoRecordExpertise records pattern insights when EventStore has tool data", async () => {
		...
		const { client, recordCalls } = createFakeMulchClient(learnResult);
		const mailDbPath = join(tempDir, ".codexstory", "insight-analysis-mail.db");

		// Create EventStore with test data
		const eventsDbPath = join(tempDir, ".codexstory", "events.db");
	...
	test("autoRecordExpertise includes insight summary in notification mail", async () => {
		...
		const { client } = createFakeMulchClient(learnResult);
		const mailDbPath = join(tempDir, ".codexstory", "insight-mail-summary.db");
...
/**
 * Tests for `codexstory log` with --stdin flag.
 *
...
describe("logCommand --stdin integration", () => {
	...
	/**
	 * Helper: run `codexstory log` as a subprocess with stdin piped.
...
`;
	...
	test("tool-start with --stdin still writes to legacy log files", async () => {
		...
		const logsDir = join(tempDir, ".codexstory", "logs", "legacy-compat-agent");
		const markerPath = join(logsDir, ".current-session");
		const markerFile = Bun.file(markerPath);
		expect(await markerFile.exists()).toBe(true);
