/**
 * Tests for codexstory hooks install/uninstall/status command.
 *
 * Uses real temp directories and real filesystem (no mocks needed).
 * Each test gets an isolated temp directory with minimal .codexstory/
 * and .codex/ scaffolding.
 */
...
/** Orchestrator hooks content for .codexstory/hooks.json. */
const SAMPLE_HOOKS = {
	hooks: {
		SessionStart: [
			{
				matcher: "",
				hooks: [{ type: "command", command: "codexstory prime --agent orchestrator" }],
			},
		],
		Stop: [
			{
				matcher: "",
				hooks: [{ type: "command", command: "codexstory log session-end --agent orchestrator" }],
			},
		],
	},
};
...
beforeEach(async () => {
	...
	// Create minimal .codexstory/ with config.yaml
...
describe("hooksCommand help", () => {
	test("--help outputs help text", async () => {
		...
		expect(output).toContain("codexstory hooks");
...
describe("hooks install", () => {
	test("installs hooks from .codexstory/hooks.json to .codex/settings.local.json", async () => {
		// Write source hooks
		await Bun.write(
			join(tempDir, ".codexstory", "hooks.json"),
		...
		const targetPath = join(tempDir, ".codex", "settings.local.json");
	...
	test("throws when .codexstory/hooks.json does not exist", async () => {
	...
	test("writes JSON with trailing newline", async () => {
		...
		const content = await Bun.file(join(tempDir, ".codex", "settings.local.json")).text();
...
describe("hooks install merge behavior", () => {
	test("--force merges codexstory hooks into existing user hooks", async () => {
		await Bun.write(
			join(tempDir, ".codexstory", "hooks.json"),
			`${JSON.stringify(SAMPLE_HOOKS, null, "\t")}\n`,
		);
...
describe("hooks status", () => {
	test("reports source missing when .codexstory/hooks.json does not exist", async () => {
		const output = await captureStdout(() => hooksCommand(["status"]));
		expect(output).toContain("missing");
	});

	test("reports installed:false when no hooks in .codex/", async () => {
		await Bun.write(
			join(tempDir, ".codexstory", "hooks.json"),
			`${JSON.stringify(SAMPLE_HOOKS, null, "\t")}\n`,
		);
		...
		expect(output).toContain("codexstory hooks install");
	});

	test("reports installed:true when hooks present in .codex/", async () => {
		await Bun.write(
			join(tempDir, ".codexstory", "hooks.json"),
			`${JSON.stringify(SAMPLE_HOOKS, null, "\t")}\n`,
		);
