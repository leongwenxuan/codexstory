/**
 * CLI command: codexstory monitor start|stop|status
 *
 ...
 * - Has no overlay CODEXSTORY.md (context comes via codexstory status + mail)
...
function monitorTmuxSession(projectName: string): string {
	return `codexstory-${projectName}-${MONITOR_NAME}`;
}
...
export function buildMonitorBeacon(): string {
	...
	const parts = [
		`[CODEXSTORY] ${MONITOR_NAME} (monitor/tier-2) ${timestamp}`,
		"Depth: 0 | Parent: none | Role: continuous fleet patrol",
		`Startup: run mulch prime, check fleet (codexstory status --json), check mail (codexstory mail check --agent ${MONITOR_NAME}), then begin patrol loop`,
	];
	return parts.join(" — ");
}
 ...
 * 2. Load config
 * 3. Deploy hooks to project root's .codex/ (monitor-specific guards)
...
async function startMonitor(args: string[]): Promise<void> {
	...
	if (!config.watchdog.tier2Enabled) {
		throw new AgentError(
			"Monitor agent (Tier 2) is disabled. Set watchdog.tier2Enabled: true in .codexstory/config.yaml to enable.",
...
const MONITOR_HELP = `codexstory monitor — Manage the persistent Tier 2 monitor agent

Usage: codexstory monitor <subcommand> [flags]

Subcommands:
  start                    Start the monitor (spawns Codex at project root)
...
The monitor agent (Tier 2) continuously patrols the agent fleet by:
  - Checking agent health via codexstory status
...
/**
 * Entry point for `codexstory monitor <subcommand>`.
 */
	...
	switch (subcommand) {
		...
		default:
			throw new ValidationError(
				`Unknown monitor subcommand: ${subcommand}. Run 'codexstory monitor --help' for usage.`,
