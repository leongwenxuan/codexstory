/**
 * CLI command: codexstory dashboard [--interval <ms>] [--all]
 *
...
function renderHeader(width: number, interval: number, currentRunId?: string | null): string {
	const left = `${color.bold}codexstory dashboard v0.2.0${color.reset}`;
	const now = new Date().toLocaleTimeString();
	const scope = currentRunId ? ` [run: ${currentRunId.slice(0, 8)}]` : " [all runs]";
	const right = `${now}${scope} | refresh: ${interval}ms`;
	const leftStripped = "codexstory dashboard v0.2.0"; // for length calculation
...
/**
 * Entry point for `codexstory dashboard [--interval <ms>] [--all]`.
 */
const DASHBOARD_HELP = `codexstory dashboard — Live TUI dashboard for agent monitoring

Usage: codexstory dashboard [--interval <ms>] [--all]
