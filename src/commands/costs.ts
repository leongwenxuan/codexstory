/**
 * CLI command: codexstory costs [--agent <name>] [--run <id>] [--by-capability] [--last <n>] [--self] [--json]
 *
...
/**
 * Discover the orchestrator's Codex CLI transcript JSONL file.
 *
 * Scans ~/.codex/projects/{project-key}/ for JSONL files and returns
...
async function discoverOrchestratorTranscript(projectRoot: string): Promise<string | null> {
	...
	const projectDir = join(homeDir, ".codex", "projects", projectKey);
...
const COSTS_HELP = `codexstory costs -- Token/cost analysis and breakdown

Usage: codexstory costs [options]
...
/**
 * Entry point for `codexstory costs [--agent <name>] [--run <id>] [--by-capability] [--last <n>] [--self] [--json]`.
 */
export async function costsCommand(args: string[]): Promise<void> {
	...
	const codexstoryDir = join(config.project.root, ".codexstory");
	...
	if (self) {
		...
		if (!transcriptPath) {
			if (json) {
				...
				process.stdout.write(
					"No orchestrator transcript found.\nExpected at: ~/.codex/projects/{project-key}/*.jsonl\n",
				);
			}
			return;
		}
	...
	if (live) {
		const metricsDbPath = join(codexstoryDir, "metrics.db");
		...
		const { store: sessionStore } = openSessionStore(codexstoryDir);
