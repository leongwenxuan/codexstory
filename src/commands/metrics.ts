/**
 * CLI command: codexstory metrics [--last <n>] [--json]
 *
...
/**
 * Entry point for `codexstory metrics [--last <n>] [--json]`.
 */
const METRICS_HELP = `codexstory metrics — Show session metrics

Usage: codexstory metrics [--last <n>] [--json]
...
export async function metricsCommand(args: string[]): Promise<void> {
	...
	const dbPath = join(config.project.root, ".codexstory", "metrics.db");
