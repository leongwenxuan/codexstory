...
describe("createMergeResolver", () => {
	...
	describe("result shape", () => {
		...
		test("failed result preserves original entry fields", async () => {
			await setupDeleteModifyConflict(repoDir, defaultBranch, "codexstory/my-agent/bead-xyz");

			const entry = makeTestEntry({
				branchName: "codexstory/my-agent/bead-xyz",
				beadId: "bead-xyz",
				agentName: "my-agent",
				filesModified: ["src/test.ts"],
			});
			...
			expect(result.entry.branchName).toBe("codexstory/my-agent/bead-xyz");
