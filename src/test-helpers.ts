...
const GIT_TEST_ENV = {
	...
	GIT_AUTHOR_EMAIL: "test@codexstory.dev",
	GIT_COMMITTER_NAME: "Overstory Test",
	GIT_COMMITTER_EMAIL: "test@codexstory.dev",
};
...
async function getTemplateRepo(): Promise<string> {
	...
	const dir = await mkdtemp(join(tmpdir(), "codexstory-template-"));
...
export async function createTempGitRepo(): Promise<string> {
	...
	const dir = await mkdtemp(join(tmpdir(), "codexstory-test-"));
	...
	await runGitInDir(dir, ["config", "user.email", "test@codexstory.dev"]);
	return dir;
}
